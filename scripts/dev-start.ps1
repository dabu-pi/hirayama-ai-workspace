...
$remote = (git remote 2>$null | Select-Object -First 1).Trim()
...
$behind = (git rev-list --count 'HEAD..@{u}' 2>$null).Trim()
if ($LASTEXITCODE -eq 0 -and $ahead -match '^\d+$' -and $behind -match '^\d+$') {
    $aheadN  = [int]$ahead
    $behindN = [int]$behind
    if ($aheadN -gt 0) { Write-Warn "まだ push されていないコミットが ${aheadN} 件あります" }
    if ($behindN -gt 0) { Write-Info "リモートより ${behindN} コミット遅れています（pull で取得します）" }
    if ($aheadN -eq 0 -and $behindN -eq 0) { Write-Ok 'リモートと同期済みです（pull 前の確認）' }
}
...
if ($dirty.Count -gt 0) {
    ...
    if ($dirty.Count -gt 10) { Write-Host "    ... 他 $($dirty.Count - 10) 件" }
    Write-Host ''
...
$visiblePullOutput = @($pullOutput | Where-Object { $_ -and $_ -notmatch '^warning:' })
$visiblePullOutput | ForEach-Object { Write-Host "  $_" }
Write-Host ''

if ($pullExit -eq 0) {
    $isUpToDate = $visiblePullOutput | Where-Object { $_ -match 'Already up.to.date|Already up to date' }
    if ($isUpToDate) { Write-Ok 'すでに最新です（変更なし）' }
    else { Write-Ok 'pull 完了！最新コードを取得しました' }
} else {
