...
- 画像は元画像を正本として保持し、新サイト表示用の700x700正方形画像は出力処理で派生生成する
...
| `product_media` | 商品ID、画像番号、元画像URL/Drive ID、表示用画像URL、表示幅/高さ、alt、並び順、代表画像フラグ、公開フラグ | 商品画像管理。元画像は縦横比自由で保持し、表示用画像は700x700正方形の派生生成物として扱う |
...
| 画像管理 | Drive フォルダ + `product_media` 台帳 + 700x700表示用画像生成、後でアップロードUI | 元画像を壊さず保持しながら、新サイト側では正方形表示を安定させやすい |
...
4. `products` + `product_media` + `channel_settings` から、元画像とは別の700x700表示用画像URLを含む新サイト用 JSON を生成する。
...
- 表示用画像の保存先、背景色、派生画像の再生成タイミング、サムネイル派生をv0で持つかどうか
...
| 領域 | 方針 | 詳細 |
|---|---|---|
| 商品正本 | `商品マスタ` を正本にし、`internal_id` と `sd_product_code` を分離する | `docs/product-master-v0.md` |
| 分類マスタ | 店舗/メーカー/部位/状態/カテゴリ/公開状態を `設定マスタ` に集約する | `docs/settings-master-v0.md` |
| 公開派生ビュー | `サイト出力ビュー` を商品マスタから生成し、一覧/詳細/絞り込み/SEO/公開判定に特化させる | `docs/site-output-view-v0.md` |
| JSON中間仕様 | `products.json` を WordPress非依存の `schemaVersion + products[]` で定義する | `docs/products-json-spec.md` |
| 変換設計 | 現行 `ネットショップ商品一覧` から新構造への列マッピング、加工/廃止/隔離保持を整理する | `docs/current-to-v0-mapping.md` |
| 商品コード互換 | 既存 `sd_product_code` は保持し、新規採番と検証を設定マスタ参照で厳格化する | `docs/product-code-validation-spec.md` |
| 競合価格 | 新サイト本体と独立した `競合価格データ` タブを置き、自社商品候補紐付けとレビュー状態を持つ | `docs/competitor-data-v0.md` |

## 次フェーズでの設計論点

| 論点 | 次に決めること |
|---|---|
| 技術構成比較 | `静的JSON + フロント`, `軽量API + DB`, `Sheets + キャッシュ` のどれをMVPにするか比較する |
| 画像配信 | 元画像URLと700x700表示用画像URLをどう分けて保存し、Drive/オブジェクトストレージ/CDNへ将来移せるようにするか決める。共通ルールは `docs/image-spec-v0.md` を参照する |
| 非公開/売却済み公開 | `publish_status` と `condition_code=sold` の運用ルールを現場確認して確定する |
| 見積統合 | 正本ブック確定後、`見積入力` / `見積履歴` の詳細列と商品マスタ参照方式を詰める |
| 並行運用 | 現行 `sendHttpPost()` を止めずに、新v0シートと `products.json` を読み取り専用で並走させる移行手順を具体化する |
