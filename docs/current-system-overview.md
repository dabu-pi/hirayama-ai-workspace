...
- ユーザー前提では「GAS からサイト側へ商品データを一括反映」しているが、今回の調査では対象シートに紐づく GAS 本体や scriptId をまだ取得できていない。
- そのため、現時点では「WordPress 用CSV生成は確認済み」「GAS からの実反映経路は未確認」と切り分ける。
...
| GAS本体の所在が不透明 | 対象シートに紐づくコンテナバインドGASの scriptId/コードを今回まだ取得できていない | 商品コード生成、サイト反映、見積自動入力、競合収集の正確な責務確定が未完 |
...
- ローカルの `freee-automation` は案件進捗台帳と連動しており、見積/請求/freee/Gmail 周辺の現行ロジックを一部担っている。

## まだ不明なこと

- 商品マスタ本体に紐づく GAS の scriptId、コード、トリガー、デプロイ状態
- WordPress への実反映が CSV 手作業なのか、GAS/API 自動投入なのか、その現行経路
