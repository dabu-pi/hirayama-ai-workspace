...
| `image_urls` | string/JSON | 任意 | 人 + GAS | `画像1`, `画像2`, `画像3` | v0 では改行区切りまたは JSON配列文字列で保持。画像専用タブ/Drive連携は将来分離余地あり |
| `main_image_url` | string | 任意 | GAS | `画像1` | 代表画像。`image_urls` の先頭から自動設定する |
...
| 区分 | 列 |
|---|---|
| 人が主に入力する列 | `product_name`, `maker_code`, `condition_code`, `store_code`, `purchase_year`, `part_code`, `category_code`, `base_price_ex_tax`, `discount_price_ex_tax`, `cost_ex_tax`, `shipping_fee_ex_tax`, `stock_quantity`, `publish_status`, `featured_flag`, `inquiry_enabled`, `description_text`, `description_html`, `search_keywords`, `size_text`, `weight_text`, `image_urls`, `purchase_date`, `supplier_name`, `sold_date`, `sold_price_ex_tax`, `sold_to`, `legacy_base_export_flag`, `remarks` |
| GASが主に生成/補完する列 | `internal_id`, `sd_product_code`, `maker_name`, `condition_label`, `store_name`, `purchase_year_code`, `part_label`, `category_label`, `sale_price_ex_tax`, `sold_out_flag`, `main_image_url`, `seo_slug`, `seo_title`, `seo_description`, `legacy_wp_post_id`, `source_sheet_name`, `source_row_no`, `created_at`, `updated_at` |
...
| 画像 | 現行 `画像1〜3` のURLを壊さず保持しつつ、将来のDrive/オブジェクトストレージ移行を見据え `image_urls` 配列表現に寄せる |
