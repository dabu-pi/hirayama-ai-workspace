"""
Run YouTube Suggest collector and persist artifacts.

Usage:
    python scripts/run_youtube_suggest.py --mode mock
    python scripts/run_youtube_suggest.py --mode live --seed-id concept2_skierg_model
    python scripts/run_youtube_suggest.py --mode auto --max-seeds 2
"""
...
from src.collectors.youtube_suggest import (
    DEFAULT_MOCK_FIXTURE,
    DEFAULT_SEED_CONFIG,
    YouTubeSuggestCollector,
)

DEFAULT_OUTPUT_ROOT = ROOT / "data" / "collected" / "youtube_suggest"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run YouTube Suggest collector. auto=prefer live then fallback to mock if live returns no observations."
    )
    ...
    batch = collector.collect_bundle(mode=args.mode, seed_ids=args.seed_ids, max_seeds=args.max_seeds)

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_root = Path(args.output_root)
    raw_path = output_root / "raw" / f"youtube_suggest_{ts}.json"
    observations_path = output_root / "observations" / f"youtube_suggest_{ts}.csv"
    import_path = output_root / "import" / f"youtube_suggest_{ts}.csv"
    ...
    print(f"[COLLECT] seeds={len(requested_seeds)} hl={requested_seeds[0].hl if requested_seeds else 'n/a'} gl={requested_seeds[0].gl if requested_seeds else 'n/a'}")
