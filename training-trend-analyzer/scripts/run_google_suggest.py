"""
Run Google Suggest collector and persist artifacts.

Usage:
    python scripts/run_google_suggest.py --mode mock
    python scripts/run_google_suggest.py --mode live --seed-id concept2_skierg_model
    python scripts/run_google_suggest.py --mode auto --max-seeds 2
"""
...
from src.collectors.google_suggest import (
    DEFAULT_MOCK_FIXTURE,
    DEFAULT_SEED_CONFIG,
    GoogleSuggestCollector,
)

DEFAULT_OUTPUT_ROOT = ROOT / "data" / "collected" / "google_suggest"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Google Suggest collector. auto=prefer live then fallback to mock if live returns no observations."
    )
    ...
    raw_path = output_root / "raw" / f"google_suggest_{ts}.json"
    observations_path = output_root / "observations" / f"google_suggest_{ts}.csv"
    import_path = output_root / "import" / f"google_suggest_{ts}.csv"
