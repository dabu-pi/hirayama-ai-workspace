...
このプロジェクトの最終到達点は、AI が複数ソースから週次トレンド候補を自動収集・集計・比較し、中古トレーニングマシン等の仕入れ判断に使える参考材料を継続的に出せる状態です。

ただし現段階の主目的は、AI が無人で仕入れを確定することではありません。優先するのは、次の 3 点です。
...
- ranking / compare は source 欠落を「需要ゼロ」と誤認すると、人の review を誤誘導する
- 仕入れ判断支援では、完全停止よりも「どこが degraded かを明示して参考情報を出す」ほうが実務価値が高い場面が多い

fallback 設計の目的は、障害を隠すことではなく、障害時でも誤解しにくい参考情報を出すことです。

## 3. 最終到達点と非ゴール

### 最終到達点

AI が複数ソースから週次トレンド候補を自動収集・集計・比較し、仕入れ判断の参考材料を継続的に出せる状態。
...
| YouTube Suggest | 第3の軽量補助 signal | unofficial endpoint, JSON parse | compare / review 補助 | 欠落時は soft fail 寄り |
...
扱い:
- ranking は継続可能
- compare は `review only` 扱い
- console に source 欠落と解釈制限を明示する
...
- seed / query 単位の失敗はできるだけ source 全停止にしない
...
| 状態 | 条件 | ranking 扱い |
|---|---|---|
| `ok` | GT が target 週にあり、補助 source も概ね利用可能 | 通常出力してよい |
| `review_only` | GT はあるが、GS / YT 欠落や metadata 欠損がある | 出力はしてよいが参考情報扱いに落とす |
| `blocked` | GT 欠落、全 source 欠落、DB 読み出し不能 | batch 全体を止める |
...
- source 欠落時の compare は「review 補助」までで、仕入れ判断の強い根拠としては扱わない

## 8. hard fail / soft fail / skip / warn の判断基準

| 判定 | 代表例 | batch の扱い | console | CSV |
|---|---|---|---|---|
| hard fail | DB 読み出し不能、GT 欠落、全 source 欠落 | 停止 | `[ERROR]` を出す | 通常 CSV は出さない |
| soft fail | GS 全欠落、YT 全欠落、source parse 失敗、mock fallback | 継続可だが `review only` | `[WARN]` と health 行を出す | row data は出してよいが summary 行は入れない |
| skip | seed 無し、query ごと 0 件、non-model seed | 一部行だけ除外 | 必要なら seed warn | CSV は通常どおり |
| warn | sample_size 欠損、metadata 欠損、partial source failure | 継続 | `[WARN]` を出す | 事故説明は sidecar 優先 |
...
1. source health の共通スキーマを決める
   collector artifact / import 後 / run_batch で同じ語彙を使えるようにする
