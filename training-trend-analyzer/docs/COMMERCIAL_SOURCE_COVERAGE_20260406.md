...
That makes the current publication-health denominator 13 ranking entities:

- 11 resolved commercial model rows with `model_id`
- 2 brand-only rows with `model_id=NULL` that currently still enter `run_batch` health as entities

## Current Result

`run_batch.py` currently reports:
...
|---|---|---:|---:|---:|---|
| GT | `google_trends_interest` | 4 | 9 | 30.8% | warning -> `review_only` |
| GS | `search_suggest_count` | 4 | 9 | 30.8% | warning -> `review_only` |
| YT | `youtube_suggest_count` | 4 | 9 | 30.8% | warning -> `review_only` |
...
| JOHNSON `(model_id NULL)` | unknown | n/a | missing | missing | missing | cleanup / resolve before collector coverage |
| TECHNOGYM `(model_id NULL)` | unknown | n/a | missing | missing | missing | cleanup / resolve before collector coverage |
...
Current `config/trends/seed_queries.json` has four `seed_type=model` entries, and they match the four covered models:
...
There are no current model seeds for the seven resolved missing models listed above.

## Shortest Route To Fill Coverage

1. Resolve the 2 brand-only rows.

   These currently inflate the health denominator to 13 but cannot be directly covered by model collectors because they have no `model_id`.

   Minimum decision needed:

   - map them to a canonical model if they are legitimate model observations, or
   - mark them out of the publication-health denominator / cleanup the source rows if they are brand-level inputs.

2. Add model seeds for the 7 resolved missing models.
...
After adding the seven model seeds to `config/trends/seed_queries.json`, use the existing collectors:
...
- brand-only rows either resolve into proper model rows or stop entering the publication-health denominator

## Notes

- The shortest true blocker is not a single source. GT, GS, and YT are all at `4/13`.
...
- The two `model_id=NULL` rows are a data hygiene blocker before full coverage can be interpreted cleanly.
