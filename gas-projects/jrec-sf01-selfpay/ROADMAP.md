...
| タスク | 内容 |
|---|---|
| 6-N-1 | 共通タブナビ HTML / CSS の整理（`index.html` または新規 `nav.html` として抽出） |
| 6-N-2 | 現在ページの active タブ表示（テンプレート変数で `currentPage` を渡す） |
| 6-N-3 | 各タブから `?page=xxx` への安全なリンク生成（`APP_URL` ベース） |
| 6-N-4 | スマホ表示確認（横スクロール型 or flex-wrap で折り返し） |
