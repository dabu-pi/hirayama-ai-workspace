...
最終更新: 2026-05-02（Phase 6-N 設計登録）
...
| デプロイ | 内容 |
|---|---|
| @29 | Phase 6-G-1 カレンダー月移動ナビ 含む（**最新本番**） |
...
| タスク | 内容 |
|---|---|
| 6-N-1 | 共通タブナビ HTML / CSS の整理（`index.html` または新規 `nav.html` として抽出） | 🔄 実機確認待ち |
| 6-N-2 | 現在ページの active タブ表示（テンプレート変数で `currentPage` を渡す） | 🔄 実機確認待ち |
| 6-N-3 | 各タブから `?page=xxx` への安全なリンク生成（`APP_URL` ベース） | 🔄 実機確認待ち |
| 6-N-4 | スマホ表示確認（横スクロール型 or flex-wrap で折り返し） | 🔄 実機確認待ち |
...
> **ステータス:** 🔄 実装済み・HEAD実機確認待ち（2026-05-02）
...
2. Phase 6-N — 共通タブナビゲーション（UI改善・最小変更）← 次期優先候補
3. Phase 6-H — dailyCheckout 日別金額合計（既存関数連携）
