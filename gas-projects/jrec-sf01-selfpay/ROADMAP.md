...
| AI-4.5-11 | RPC null 問題対応（HEAD /dev で `raw response object null` を観測）: `toRpcSafeObject_` 追加で全 return を JSON 安全化 / `_rpcSafeStringArray_` で rawHeaders を String[] 化 / `debug.normalizedHeaders` 追加 / `assessment.createdAt` を ISO 文字列化 / `debugAI45RpcPing` + `getLatestAIAssessmentForVisitOrPatientJson` 診断関数追加 / フロントで ping & JSON-string fallback を並行発射（6回目 clasp push 2026-05-12 / server=local 完全一致を `clasp pull` で再確認） | ✅ |
