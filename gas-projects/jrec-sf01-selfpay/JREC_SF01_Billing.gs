...
// ============================================================
// TEMPORARY — データ補正（実行後に削除する）
// 対象: SPV_20260428_P0001_007
// 原因: Phase 5-B Step 2 展開前(@19)に旧UIで誤って全額入金済み扱いになった
// 補正内容: paymentStatus→未収 / paidAmount→0 / billingStatus→未収
// 実行方法: Apps Script エディタで correctPayment_P0001_007() を選択して「実行」
// 確認後、この関数を削除して clasp push すること
// ============================================================
function correctPayment_P0001_007() {
  var VISIT_KEY = "SPV_20260428_P0001_007";
  var REASON    = "旧UI(@19)誤操作: collectedAmount なしで全額入金済に更新されたため未収に戻す";
  Logger.log("[correctPayment_P0001_007] START visitKey=" + VISIT_KEY);
  ...
  try {
    ...
    var rows     = paymentSh.getRange(2, 1, paymentSh.getLastRow() - 1, 11).getValues();
    var rowIndex = -1;
    var oldStatus = "", oldPaid = "", oldDate = "", oldMemo = "";
    for (var i = 0; i < rows.length; i++) {
      if (String(rows[i][1]) === VISIT_KEY) {
        rowIndex  = i + 2;
        oldStatus = rows[i][6] || "";
        oldDate   = rows[i][7] || "";
        oldMemo   = rows[i][8] || "";
    ...
    if (rowIndex === -1) {
      Logger.log("[correctPayment_P0001_007] ERROR: not found in Payments");
      return;
    }
    Logger.log("[correctPayment_P0001_007] BEFORE status=" + oldStatus +
      " paidAmount=" + oldPaid + " date=" + oldDate + " memo=" + oldMemo);

    paymentSh.getRange(rowIndex, 7).setValue("未収");    // paymentStatus
    paymentSh.getRange(rowIndex, 8).setValue("");         // paymentDate クリア
    paymentSh.getRange(rowIndex, 11).setValue(0);         // paidAmount
    paymentSh.getRange(rowIndex, 9).setValue(
      "【訂正 " + today + "】" + REASON + " / 旧値: status=" + oldStatus + " memo=" + oldMemo
    );
    Logger.log("[correctPayment_P0001_007] Payments row=" + rowIndex + " → 未収 / paidAmount=0");

    // ── SelfPayVisits.billingStatus 修正 ──────────────────
    var vKeys = visitSh.getRange(2, 1, visitSh.getLastRow() - 1, 1).getValues();
    var visitFixed = false;
    for (var j = 0; j < vKeys.length; j++) {
      if (String(vKeys[j][0]) === VISIT_KEY) {
        visitSh.getRange(j + 2, 9).setValue("未収");
        Logger.log("[correctPayment_P0001_007] SelfPayVisits row=" + (j + 2) + " billingStatus→未収");
        visitFixed = true;
        break;
      }
    }
    if (!visitFixed) Logger.log("[correctPayment_P0001_007] WARN: visitKey not found in SelfPayVisits");

    // ── Run_Log に記録 ────────────────────────────────────
    runLogSh.appendRow([
      new Date(), "CORRECTION", VISIT_KEY,
      VISIT_KEY.split("_")[2] || "",
      "SUCCESS",
      "visitKey: " + VISIT_KEY + " " + REASON,
      "manual"
    ]);

    Logger.log("[correctPayment_P0001_007] DONE — Run_Log に記録済み");
    Logger.log("[correctPayment_P0001_007] 確認: Payments row=" + rowIndex +
      " の col7=未収 / col8=空 / col11=0 / col9=訂正メモ を確認してください");

  } catch(e) {
    Logger.log("[correctPayment_P0001_007] ERROR: " + e.message);
  }
}
