...
**✅ Phase Chart-Ref-1 初回・前回カルテ参照パネル: CLOSED（2026-05-12 @41 本番反映）**
  ...
  **HEAD /dev 実機確認 PASS（2026-05-12）:**
    ...
    - 2回目以降 visit: 「過去カルテ参照」カード表示・初回カルテの visitKey / 主訴・症状 / 評価 / 所見 / 施術内容 / 説明内容が表示・read-only・自動コピーなし
    - AI-4.5 への影響: なし（保存済みAI評価再表示は引き続き正常）
  ...
  **次フェーズ候補（優先度順）:**
    1. Phase Chart-Ref-2 — 必要項目だけ「引用」できるボタン追加（手動コピー）
    2. Phase AI-5 — 過去判定比較・初回 / 前回との差分・改善 / 悪化傾向
...
HEAD /dev で `found:true / reason: "match found" / vkMatches: 5` を確認し @40 本番反映）

**deploy 履歴:**
- @39 (2026-05-12): 当初 CLOSED 記録 → 追加不具合判明により再オープン → 後継 @40 が本命
- @40 (2026-05-12): header tolerant read + debug payload + RPC 安全化（本命・現運用版）
  ...
  **追加修正（6回目 clasp push, 2026-05-12 / RPC null 問題対応）:**
  - 観測（HEAD /dev での再検証）:
    ```
    ...
    ```
    GAS 関数の戻り値が **そのまま `null`** で client に届いている状態
    （全 return path で plain object を返す実装にも関わらず）
  - 推定原因: HtmlService RPC のシリアライザが、応答 object 内に Date / undefined / 特殊型が
        含まれていると silent に `null` 化して返す既知の挙動
  - 該当しそうな箇所:
        * `debug.rawHeaders = data[0]` — `getValues()` の生 array は Date / number / null が混入し得る
        * `assessment.createdAt = target[idxAt]` — Date object 直接返却
  - 修正:
        * `toRpcSafeObject_(obj)` を追加 — JSON.parse(JSON.stringify(...)) で undefined→null, Date→ISO, Error→stack 文字列に正規化
        * `_rpcSafeStringArray_(arr)` を追加 — getValues 行を確実に String[] に変換
        * `debug.rawHeaders` を String[] に変換してから格納
        * `debug.normalizedHeaders` を追加（trim+lowercase 後の配列）
        * `assessment.createdAt` を ISO 文字列に変換
        * `assessment.patientId` を追加（フロント側で見えるように）
        * すべての return を `toRpcSafeObject_(...)` 経由に統一
  - 診断関数追加:
        * `debugAI45RpcPing()` — シート参照なしで固定 object を返す。RPC 層の健全性確認用
        * `getLatestAIAssessmentForVisitOrPatientJson(visitKey, patientId)` — 本体結果を JSON 文字列で返す（文字列は確実に serializable）
  - フロント側 (visit-form.html):
        * `loadSavedAIAssessment` 内で診断 ping と JSON-string fallback を並行発射
        * Console に `[AI45] rpc ping response` / `[AI45] JSON-string fallback raw/parsed` が追加で出る
        * これで「object が null」「string で取れる」場合は RPC 層の問題、両方 null なら別問題、と一意に切り分け可能
  - サーバー verify: `clasp pull` で server == local 完全一致を確認済み
        * toRpcSafeObject_                       (line 604)
        * _rpcSafeStringArray_                   (line 630)
        * debugAI45RpcPing                       (line 783)
        * getLatestAIAssessmentForVisitOrPatientJson (line 811)

  **追加修正（5回目 clasp push, 2026-05-12 / 診断 payload 拡充）:**
  - 観測: 4回目 push 後も Console は `{ ok: false, found: false }`。
        ただし local↔server の `clasp pull` 差分は **完全に 0**（サーバーには新ロジック反映済み）。
        重複関数定義もなし。@HEAD deploymentId と @39 frozen deploymentId が別物なので
        `@39 の deploymentId で /dev を叩いていた可能性`が極めて高い（versioned deployment の /dev は HEAD を serve しないケースがある）。
  - 検証: `clasp pull` の結果、
        - JREC_SF01_Main.gs (34154 bytes) — local と完全一致
        - visit-form.html — local と完全一致
        - debugAIAssessmentsRead は line 692 に確実に存在
        - 単一定義のみ・shadow なし
  - 真の原因（推定）の最有力は **URL 取り違え** だが、それでも対応として診断 payload を拡充:
  - 修正: `getLatestAIAssessmentForVisitOrPatient` の返却 schema を `{ ok, found, assessment, debug }` に変更
        - 失敗系も含めて常に `debug.reason` を返す:
          `"match found"` / `"no match"` / `"header missing"` / `"sheet not found"` / `"sheet empty"` / `"exception"`
        - `debug.sheetName / spreadsheetId(先頭8文字) / headerCount / rowCount / rawHeaders / missingHeaders` を返却
        - `debug.needleVk / needlePid / totalScanned / vkMatches / pidMatches` を返却
        - 例外時は `debug.error` も返却
  - 修正: visit-form.html のフロント側
        - `[AI45] raw response object` / `[AI45] raw response JSON` で生 response を必ず出力
        - `[AI45] getLatestAIAssessmentForVisitOrPatient summary` で debug payload を平坦化して出力
        - 判定を `res.found` に変更（旧: `res.ok && res.assessment`）— ok と found を分離
  - 既存の保存・PII 除外ポリシーには変更なし
  - **重要な運用注意:** 検証は **HEAD deploymentId の /dev URL** で行う必要がある:
    - HEAD: `AKfycbzJWJAKCxStP82lfFl8eEHei98dWh7f6cgtEM33r3M5`
    - @39:  `AKfycbxdngcgHbq4q52xPV3-ukDlun3s29Kbk8gy_oA11RGrwe4oPDBr4ocbeRyGUys8oxMCpw`（これは frozen / `/exec` 用）
    - /dev URL: `https://script.google.com/macros/s/AKfycbzJWJAKCxStP82lfFl8eEHei98dWh7f6cgtEM33r3M5/dev?page=visitForm&id=P0001&visitKey=SPV_20260511_P0001_001`
  - 次工程: HEAD /dev で PASS 確認 → versioned deploy @40 → 改めて CLOSED 化

**Phase AI-4.5 実機確認結果（最終 2026-05-12 @40 PASS）:**
  - ✅ AI_Assessments v2保存: PASS
  - ✅ outputJson.aiImpression保存: PASS
  - ✅ promptVersion=v2: PASS
  - ✅ 保存済みAI評価自動再読込: PASS（HEAD /dev で `found:true` / `reason:"match found"`）
  - ✅ 青バナー表示: PASS
  - ✅ カルテ下書き再表示: PASS
  - ✅ AI参考見立て再表示: PASS
  - ✅ 新規AI実行: PASS（緑バナー・freshResult 上書き正常）

**HEAD /dev 最終確認（2026-05-12）:**
  ...
  - `[AI45] rpc ping response object`: ✅ PASS
  - `[AI45] JSON-string fallback raw / parsed`: ✅ PASS
  ...
  - `displaySavedAssessment called`: ✅ PASS
  - `displaySavedAssessment rendered banner`: ✅ PASS

**versioned deploy @40（本命・現運用版）: 本番反映済み（2026-05-12）:**
