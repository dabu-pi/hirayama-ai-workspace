...
| タスク | 対象ファイル | 内容 |
|---|---|---|
| AI-2-1 | visit-form.html | AI補助判定セクション追加（カルテ保存後に有効化されるボタン＋表示枠） |
| AI-2-2 | visit-form.html | カルテ保存前はAIボタンを disabled に制御（visitKey 未確定のため） |
| AI-2-3 | visit-form.html | AI出力の表示枠（summary / 問診不足 / 検査提案 / 赤旗 / 記録不足 / disclaimer）を作る |
| AI-2-4 | styles.html | AI補助判定セクション用のスタイル追加（赤旗=赤カード、注意=黄カード等） |
