...
**ステータス:** ✅ Step 1 CLOSED / ✅ Step 2 CLOSED / ✅ DailySales 反映確認 CLOSED（2026-04-28）

---

## 1. 現状調査

### 1-1. Payments シートの列構造（11列）✅ Step 1 対応済み
...
| 11 | **paidAmount** | integer | **実際の入金済み累積額（Step 1 追加）** |

**後方互換フォールバック（既存行）:**
- col 11 = 空 かつ paymentStatus=入金済 → paidAmount = totalTaxInc として処理
- col 11 = 空 かつ paymentStatus=未収/一部入金 → paidAmount = 0 として処理

### 1-2. 一部入金フロー（Step 1 実装後）✅

```
billing-form.html
  → paymentStatus = "一部入金" 選択
  → 「今回入金額」入力欄が表示される（paidAmountSection）
  → 入力中: calcRemaining() で未収残額をリアルタイム表示
  → バリデーション: paidAmount <= 0 → エラー / paidAmount >= totalTaxInc → エラー
  → savePaymentWithItems(payload) ← payload.paidAmount を含む
    ・paidAmount = payload.paidAmount（例: ¥2,000）
    ・paymentStatus → "一部入金" のまま保存
    ・Payments.col11 に paidAmount を記録
    ・SelfPayVisits.billingStatus → "未収"（一部入金も未収扱い）
  → Payments 行: totalTaxInc=3850, paidAmount=2000, paymentStatus="一部入金"
...
## 2. ✅ 解決済みの問題点（Step 1 + Step 2 + DailySales バグ修正）
...
| DailySales DUPLICATE_LOG が分割 PAYMENT_COLLECT をブロック | commit 294db4f: PAYMENT_COLLECT は seenAmountKeys チェックなし | ✅ CLOSED |
| DailySales PAYMENT_SAVE が現在の paymentStatus を参照して二重計上 | commit 294db4f: detail「入金額: ¥N」でログ当時の実受取額を判定 | ✅ CLOSED |

### DailySales 実機確認（2026-04-28）✅ PASS

| 項目 | 期待 | 実測 |
|---|---|---|
| paymentSaveTotal | ¥9,350（P0001_005 + P0001_006）| ¥9,350 ✅ |
| paymentCollectTotal | ¥5,500（¥1,000 + ¥4,500）| ¥5,500 ✅ |
| totalSales | ¥14,850 | ¥14,850 ✅ |
| 旧UI誤操作ログ「回収額: ¥5500」| MISSING_COLLECTED_AMOUNT として除外 | ✅ |
| Step 2 分割回収 2件 | rows に両方が入る | ✅ rowsCount=4 |
...
実装しなかったこと:
- collectOutstandingPayment の collectedAmount（Step 2 へ）
- 領収書の部分発行
- 既存データの補正スクリプト
```
...
```
Phase 5-B Step 2: collectOutstandingPayment に collectedAmount 実装

対象:
1. JREC_SF01_Billing.gs の collectOutstandingPayment:
   - payload.collectedAmount を受け取る
   - Payments.paidAmount += collectedAmount で累積更新
   - paidAmount >= totalTaxInc なら paymentStatus → 入金済
   - paidAmount < totalTaxInc なら paymentStatus → 一部入金
   - Run_Log detail に「今回回収額: ¥N 累積入金額: ¥M 残額: ¥K」を記録
2. receipt.html の未収回収 UI に「今回回収額」入力フィールドを追加
3. 回収後に画面の paidAmount / remainingAmount / paymentStatus をリアルタイム更新
4. JREC_SF01_DailySales.gs の paymentCollectTotal を collectedAmount ベースに修正

実装しないこと:
- 領収書の部分発行
- PaymentCollections 新規シート（案 C で累積管理）
- 既存データの補正スクリプト
...
| 2026-04-28 | Step 1 実装完了（commit 79222f7）: Payments col 11 paidAmount 追加・billing-form.html 一部入金 UI・後方互換フォールバック対応。ドキュメントをステップ進捗に合わせ更新 |
