...
## 2026-05-12 追記（最終） — HEAD /dev PASS + @41 deploy → CLOSED
...
| 確認項目 | 結果 |
|---|---|
| 初回 visit:「過去カルテ参照」カード表示 | ✅ PASS |
| 初回 visit:「初回来院のため参照なし」黄色注意カード | ✅ PASS |
| 初回 visit: 当日の入力欄は自動コピーされていない | ✅ PASS |
| 2回目以降 visit:「過去カルテ参照」カード表示 | ✅ PASS |
| 2回目以降 visit: 初回カルテの visitKey / 主訴・症状 / 評価 / 所見 / 施術内容 / 説明内容が表示 | ✅ PASS |
| 2回目以降 visit: read-only / 当日の入力欄は自動コピーされない | ✅ PASS |
| AI-4.5 保存済みAI評価再表示への影響 | ✅ なし（青バナー引き続き表示）|

### versioned deployment @41
...
```
https://script.google.com/macros/s/AKfycbz_igUXXGGd68GyHSmpmGOUBrKxDisFOMfOpwZK7fgwb3r-DsWarfmWuWmfiSwQYf9h/exec?page=visitForm&id=P0001&visitKey=SPV_20260511_P0001_001
```

確認観点（HEAD /dev と同等であることを確認するだけで足りる）:
- 白画面にならない
- 患者サマリー直下に「過去カルテ参照」カード
- 初回 / 前回 が見える（または「初回来院のため参照なし」）
- AI-4.5 が引き続き正常

### Phase Chart-Ref-1 最終状態: ✅ CLOSED

実装〜本番反映までの工程:
- 設計 + 実装（getChartReferencesForVisit / buildPage_ 1行追加 / visit-form.html scriptlet）
- clasp push: 1回
- versioned deploy: @41 を1回
- 本命 deployment: @41
