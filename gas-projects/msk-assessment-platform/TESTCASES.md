...
| TC-J02 | C22=あり（排尿障害） | 【緊急】馬尾症候群疑い — 即日医療機関紹介を強く検討してください | 未実施 | — | 最優先・緊急（branch1） |
| TC-J03 | C16=あり（外傷）/ C22〜C23=なし | 赤旗あり — 施術前に医療連携の必要性を確認してください | 未実施 | — | 赤旗1件・馬尾なし（branch2） |
| TC-J04 | C31=あり（下肢筋力低下）→ C33=重度（自動） | 神経症状重度 — 神経所見優先・整形外科精査（MRI等）を強く推奨 | 未実施 | — | NERVE_SEVERE（branch3） |
| TC-J05 | C32=陽性（右）→ C33=中等度（自動）/ C31=なし | 神経根障害疑い — 神経所見優先・施術強度を慎重に。悪化時は即医療連携 | 未実施 | — | NERVE_MOD（branch4）※両側陽性は重度になるため片側で確認 |
| TC-J06 | STarT=7（C56〜C62=全てはい(1)）/ NRS=6 | 行動変容・説明優先 — 心理社会的介入と疼痛教育を施術と並行して実施 | 未実施 | — | START_HIGH+NRS_MID（branch5） |
| TC-J07 | STarT=7（C56〜C62=全てはい(1)）/ NRS=2 | 行動変容優先 — 段階的な機能改善プログラムを導入。セルフエフィカシー（自己効力感）の向上を重視 | 未実施 | — | START_HIGH+NRS_LOW（branch6） |
| TC-J08 | NRS=8 / RMDQ=6（C42〜C47=全てはい(1)）/ STarT低 | 疼痛管理優先 → 機能改善 — まず痛みを緩和してから機能回復へ段階的に移行 | 未実施 | — | NRS_HIGH+RMDQ_MOD（branch8） |
| TC-J09 | 発症2週未満 / NRS=5 / STarT低 | 急性期管理優先 — 安静・保護と急性期疼痛管理から開始（2〜4週で改善を目標） | 未実施 | — | ACUTE+NRS_MID（branch11） |
| TC-J10 | 発症6週〜3か月 / STarT=2 / NRS=3 | 機能改善・運動療法開始 — 段階的なエクササイズと日常活動の再開を促進 | 未実施 | — | SUBACUTE+STarT低（branch12） |
