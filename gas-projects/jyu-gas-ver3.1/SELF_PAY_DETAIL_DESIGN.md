...
## Phase 2 実機確認状況（2026-03-23 完了）

> **✅ T2-1〜T2-12 全PASS（2026-03-23 院長実機確認）**

### 全テスト結果

| # | テスト内容 | 実機確認結果 |
|---|---|---|
| T2-1 | 自費1件保存 | ✅ **PASS** — 自費明細1行 / D7=メニュー名 / F7=5500 / H8="1件保存済" / 来院ヘッダ更新 確認 |
| T2-2 | 自費複数件保存 | ✅ **PASS** — 自費明細2行 / D7="〇〇ほか1件" / F7=合計 / H8="2件保存済" 確認 |
| T2-3 | 保存後 再保存（金額変更） | ✅ **PASS** — 旧行全削除→新行追記 / D7/F7/H8 再計算 正常 |
| T2-4 | 価格マスタ変更後の過去データ確認 | ✅ **PASS** — 保存済み unitPrice は snapshot で固定。JBIZ価格変更後も過去行変化なし。ダイアログ再開は最新価格を表示（設計どおり）|
| T2-5 | 会計区分=保険のみ・自費なし | ✅ **PASS** — 自費明細なし / D7/F7 空欄 / 警告なし |
| T2-6 | clearEntryUI_V3 実行後 | ✅ **PASS** — D7/F7 空欄 / B7/B8/D8/F8/H8 クリア / 自費明細シート変化なし |
| T2-7 | delete & replace 後の行番号ずれなし | ✅ **PASS** — 後方ループ削除でインデックスずれなし |
| T2-8 | 自費明細シートが存在しない場合 | ✅ **PASS** — `insertSheet` で自動作成 正常 |
| T2-9 | 自費のみ + 保険継続 case あり | ✅ **PASS** — confirm 警告 / 保険算定なし / 施術明細保険記録なし（来院ヘッダ追記は仕様として許容）|
| T2-10 | 自費のみ + 保険データなし | ✅ **PASS** — 警告なし / 自費明細のみ保存 / 来院ヘッダ保険列=0 |
| T2-11 | 保険のみ + H8="1件保存済" | ✅ **PASS** — confirm 警告 / OK後: 保険通常保存 / ヘッダ自費金額=0 |
| T2-12 | 保険+自費（通常ケース） | ✅ **PASS** — 両方保存 / ヘッダ保険列+自費列ともに正常値 |

### ✅ 修正済みブロッカー（参考記録）

| 項目 | 内容 | 対応 |
|---|---|---|
| selfPayDialog 2回目ハング | `existItems` に生 Date が含まれ JSON シリアライズ失敗 | ✅ 修正済み（2026-03-22）|
| 自費のみ来院の申請書混入リスク | `Ver3_transferData.js` に accountingType フィルタなし | ✅ 修正済み（commit `f686dba`）|
| JBIZ シート名不一致 | 「メニューマスタ（価格設定）」≠ 実名「価格設定」 | ✅ 修正済み（候補配列方式）|
| menu_id 列 O列→C列移行 | JBIZ_COL.menuId=14→2、横断影響チェック完了 | ✅ 完了（2026-03-23）|
