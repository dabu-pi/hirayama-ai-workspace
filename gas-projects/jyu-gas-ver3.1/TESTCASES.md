...
> **実装完了: 2026-04-20** — Cloud Run revision 00001-54g デプロイ済み
...
| 項目 | 内容 |
|---|---|
| 書込セル | E56（E56:AC56 マージ、単一書込） |
| 書込値 | 申請対象月の当月末日 例: 2026年4月 → `令和8年4月30日` |
| 形式 | 全角文字列でE56へ直接書込（既存ラベル文字列を上書き） |
| 公式根拠 | **未確認（暫定運用）** |
...
- `write_application.py`: `write_application()` D6 ブロック追加  
  → `calendar.monthrange` で月末日算出 → `to_wareki()` で和暦変換 → `put("E56", ...)` で書込
...
- [ ] B案実行 → xlsx を開いて E56 に当月最終日が入ること確認
