...
最終更新: 2026-04-20（TC-B01〜B06 全件クローズ記録整合完了）
...
> 対象差分: D1（施術終了年月日が未転記）
> ✅ **実質完了・クローズ（2026-03-19）** — SPEC.md D1 セクションで実ファイル確認済み
> ⚠️ 「施術証明欄の施術期間」は**別欄・別課題**（本 TC-B01 のスコープ外）

### 実装内容（確定済み 2026-03-19）

`Ver3_transferData.js` — `V3TR_buildTransferRow_` 内:

```javascript
// 継続中は部位別最終施術日、終了時は終了日優先
...
|---|---|
| 施術継続中 | 当月の当該部位の最終施術日 |
| 治癒・中止・転医 | 来院ケースの `施術終了日_部位N`（終了日優先）|
...
> 対象差分: D2（継続月数・頻回）
> ✅ **設計確定・クローズ（2026-03-20）** — M31出力は意図的停止。正本=摘要欄+長期欄（手動運用）
> SPEC.md §22 D2 に制度定義・運用方針・コード状態が全記載済み

### 設計確定内容（2026-03-20）

| 出力先 | 方針 |
|---|---|
| **M31（経過欄）** | **空欄**（意図的停止。`row["経過"]=""` 常時空） |
| **摘要欄** | 術者が手動記入（`rawContMonths` を参照）。頻回成立時は必記 |
| **長期欄** | 術者が手動：長期のみ→ 0.75 / 長期かつ頻回→ 0.5 |

### 制度定義（確定）

| 項目 | 定義 |
|---|---|
| 継続月数 | 月10回以上の施術を行った**連続月数**（単純経過月数ではない）|
| 起算ルール | 初検日1〜15日 → その月を1か月目 / 初検日16日以降 → 翌月を1か月目 |
| 頻回の成立 | 月10回以上が5か月連続 → 翌月（6か月目）から0.5逓減 |
| 頻回開始後 | 月10回未満になっても治癒・中止・転医まで0.5継続（解除不可）|

### コード状態

| ファイル | 状態 |
|---|---|
| `Ver3_transferData.js` | `V3TR_calcD2Keizoku_()` 実装済み（`rawContMonths` / `freqStarted` を返す）。本体フローでは呼ばれない（コメントアウト） |
| `Ver3_transferData.js` | `row["経過"] = ""` 常時（M31出力停止中） |
| `write_application.py` | `if keizoku:` ガードで自然スキップ（空欄のまま） |
...
M31 自動出力を復活させる場合: `Ver3_transferData.js` の `row["経過"]` 代入コメントアウト部を有効化するのみ（コードは保持済み）。
...
> 対象差分: D3（負傷名の左右表記粒度）
> ✅ **運用ルール明文化・クローズ（2026-04-20）** — コード変更なし
> 制度根拠: 左右の記載は義務ではない（左右が異なる負傷が混在する場合のみ必記）

### 結論

| 判断 | 根拠 |
|---|---|
| コード変更不要 | `V3TR_buildInjuryLabel_ = bui + " " + byomei` のパススルーで十分 |
| 実出力確認済み | `右肩関節打撲`（右 あり・正常）/ `腰部捻挫`（体幹・左右不要・正常）/ `肩関節打撲`（左右なし・制度上問題なし）|
| 制度根拠 | 「左右の記載は義務ではない。左右が異なる負傷が混在する場合のみ必記」（SPEC.md §22 D3）|
| 選択肢 B 見送り | 来院ケースへの「左右」列追加は高工数・不要。SPEC.md §23 は将来のP3/P4設計案 |
...
| 四肢・左右が区別できる部位 | `右足関節` / `左肩関節` のように左右を含めて入力 |
| 両側が同時に傷病の場合 | **必ず左右を明記**（制度上、左右が異なる負傷が混在する場合は必記）|

システムは入力値をそのままパススルーするため、入力側のルール遵守のみで対応完了。

---

## TC-B04: 負傷の原因欄の出力条件修正

> 対象差分: D4（2部位請求でも負傷原因が出力されるバグ）
> **不具合修正完了: 2026-04-20** — デプロイ済み（Cloud Run revision 00024-5gp）

### バグ内容

case1(1部位) + case2(1部位) = 2部位なのに、旧条件 `row2["部位1_計"] > 0` が
row2 の存在だけでトリガーし、負傷原因が出力されていた。
...
|---|---|
| 1〜2部位 | **空欄**（出力しない）|
| 3部位以上 | 全ケースの負傷原因を出力（（1）（2）番号付き）|

### 部位カウント方法

```
total_parts = スロット数（部位N_計 > 0 のもの）
  スロット1 = row1.部位1
  スロット2 = row1.部位2
  スロット3 = row2.部位1（row2 があるとき）
  スロット4 = row2.部位2（row2 があるとき）
part3_has_data = total_parts >= 3
```

### ローカルテスト結果（2026-04-20）

| ケース | row1 | row2 | total_parts | 期待値 | 実際 |
|---|---|---|---|---|---|
| case1(2部位)+case2(1部位) | 部位1✓部位2✓ | 部位1✓ | 3 | TRUE | ✅ TRUE |
| case1(1部位)+case2(1部位) | 部位1✓部位2✗ | 部位1✓ | 2 | FALSE | ✅ FALSE |
| case1(2部位)+row2なし | 部位1✓部位2✓ | None | 2 | FALSE | ✅ FALSE |
| case1(1部位)+row2なし | 部位1✓部位2✗ | None | 1 | FALSE | ✅ FALSE |
| case1(2部位)+case2(2部位) | 部位1✓部位2✓ | 部位1✓部位2✓ | 4 | TRUE | ✅ TRUE |

### 実機確認（院長実施）✅ 2026-04-20 完了

- [x] 2部位患者: B案実行 → 申請書 xlsx の BR21 が空欄であることを確認
- [x] 3部位患者（存在する場合）: BR21 に負傷原因テキストが入ることを確認
...
- [x] 設定シートに「施術者氏名」行を確認（なければ追加）
- [x] B案実行 → xlsx を開いて L58/L59/L62 に値が入ることを確認
- [x] 委任欄が空欄のままであることを確認

---

## TC-B06: 施術証明欄・日付の自動入力（暫定運用）

> 対象差分: D6（施術証明欄の日付 — 申請対象月の当月最終日）
> **修正完了: 2026-04-20** — Cloud Run `jrec-appgen-server` revision 00025-4lc 正サービスへ再デプロイ済み
> ⚠️ **公式根拠未確認（暫定運用）** — 厚労省・公費一次資料で「月末日記載」の明示ルール未確認。院長判断で暫定採用。

### セル再確認結果（2026-04-20）

テンプレート全行スキャン・openpyxl 実証により確認:

| 項目 | 内容 |
|---|---|
| 書込セル | **E56**（E56:AC56 マージ top-left、単一書込） |
| 書込値 | `令和X年Y月Z日`（例: `令和8年4月30日`） |
| ユーザー表示行 | 申請書の **54行目**（L58=所在地が56行目に見えるため+2オフセット） |
| 根拠 | openpyxl でテンプレートをスキャン、E56 がmaerge top-left 確認済み。ローカル write テスト PASS |
| 公式根拠 | **未確認（暫定運用）** |
...
- GAS の `APPGEN_ENDPOINT` が指す正サービス: `jrec-appgen-server`（Cloud Run ログで 06:12 に本番トラフィック確認）
- 誤デプロイ先: `write-application`（GAS から参照されない別サービス、/generate リクエストはゼロ）
- D6 コード実装自体（セル E56・ロジック・ログ）に問題はなく、デプロイ先の誤りが唯一の根本原因
- 対処: `gcloud run deploy jrec-appgen-server` で正サービスへ revision 00025-4lc をデプロイ完了

### 実装内容（2026-04-20 確定版）

- `write_application.py`: `import calendar` 追加
- `write_application.py`: `batch_write_from_string` の `clinic_info` dict に `"month"` 追加
- `write_application.py`: D6 ブロック: `calendar.monthrange` + `to_wareki()` → `ws["E56"] = date_str`
- `write_application.py`: `[D6-DATE] DONE` / `[D6-DATE] SKIP` ログ追加（Cloud Run ログで確認可能）
- GAS 側変更なし（`month` は NDJSON meta から Python 側で取得済み）
...
| `"2026-04"` | `令和8年4月30日` |
| `"2026-02"` | `令和8年2月28日` |
| `"2028-02"` | `令和10年2月29日`（うるう年） |

### 実機確認（院長実施）✅ 2026-04-20 完了

- [x] B案実行 → xlsx を開いて **E56**（ユーザー表示行54）に当月最終日が入ること確認
- [x] 書式: `令和X年Y月Z日` の文字列が入力されていること確認
...
> **実機確認: ✅ 2026-04-21 PASS**
> 確認患者: 4月 再検あり・case2あり患者 1名
> 4/1 case1初検 / 4/19 case2初検（同月）→ 4/19の初検料等=再検料410のみ（case2初検料=0 抑制確認）
...
> **実機確認: ✅ 2026-04-21 PASS**
> 確認患者: 4月 再検あり・case2あり患者 1名
> 4/1 初検（初検料等1650）/ 4/19 再検（初検料等410・後療料505）→ 再検料が初検料等欄に正しく表示
...
> **実機確認: ✅ 2026-04-21 PASS**
> 確認患者: 4月 再検あり・case2あり患者 1名
> 負傷名②に「腰部 捻挫」が表示されたことを確認（case2の負傷名が正しく出力）
...
> **実機確認: ✅ 2026-04-21 PASS（シナリオB相当）**
> 確認患者: 4月 再検あり・case2あり患者 1名
> 頸部捻挫・腰部捻挫ともに「年　月　日」の空白表示 → 継続中患者として正常動作確認
...
> **実機確認: ✅ 2026-04-21 PASS**
> 確認患者: 4月 再検あり・case2あり患者 1名
> 表面の負傷日時・場所・状況・初検時所見 = 4/1発生（頸部捻挫・case1）の情報
> case2情報が表面主表示になっていないことを確認
