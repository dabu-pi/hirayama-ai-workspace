...
### 入力
- 患者: P001
- 2026/02/03 caseKey=A 新規（初回）、傷病=捻挫、部位=腰部、受傷日=2/03
...
| 確認項目 | 期待値 |
|---|---|
| 施術明細に追加された行数 | 1行（累計2行） |
| detailID | `P001_2026-02-06_C1_P1` |
| 区分 | `後療` |
| 基本料_確定 | 505（後療料_捻挫） |
| 行合計_確定 | 505 |

**冪等性確認:**
- 同じ保存を再実行しても施術明細の行数は増えない
- 来院ヘッダの二重登録禁止エラーが先に発生するため、実質的に冪等が保証される
...
### 入力
- 患者: P001
- 2026/02/03 caseKey=A ケーススロット1（初検算定済）、傷病=捻挫、部位=腰部、受傷日=2/03
- 2026/02/10 caseKey=B ケーススロット2（同月別ケース初回）、傷病=打撲、部位=肩関節、受傷日=2/10
...
**2/10 保存後（caseB をケーススロット2に入力した場合）:**
| 確認項目 | 期待値 |
|---|---|
| detailID | `P001_2026-02-10_C2_P1` |
| caseNo | `2` |
| 区分 | `後療`（同月初検抑制による） |
| 部位 | `肩関節` |
| 傷病 | `打撲` |
...
> caseNo は UIのケーススロット番号（1 or 2）。caseKey=B を スロット1 に入力した場合は detailID が `_C1_P1` になる。
...
### 入力
- 患者: P001
- 2026/01/20 caseKey=A ケーススロット1（1月初検）、傷病=捻挫、部位=腰部、受傷日=1/20
- 2026/02/02 caseKey=B ケーススロット1（2月別ケース初回）、傷病=打撲、部位=肩関節、受傷日=2/02
...
| 確認項目 | 期待値 |
|---|---|
| detailID | `P001_2026-02-02_C1_P1` |
| caseNo | `1` |
| 区分 | `初検` |
| 部位 | `肩関節` |
| 傷病 | `打撲` |
| 基本料_確定 | 760（施療料_打撲、初検日） |
| 行合計_確定 | 760 |

**月またぎ確認:**
- 1/20 の施術明細（detailID: `P001_2026-01-20_C1_P1`）は独立して存在し、2/2 の書込みに影響されない
- 2月の初検料が正しく算定される（1月分の算定履歴が2月に持ち越されない）
