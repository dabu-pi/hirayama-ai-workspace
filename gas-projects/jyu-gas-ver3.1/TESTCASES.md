...
最終更新: 2026-04-20（TC-B06 根本原因特定 — 誤サービスデプロイ + 正サービス `jrec-appgen-server` revision 00025-4lc 再デプロイ完了）
...
- [ ] 2部位患者: B案実行 → 申請書 xlsx の BR21 が空欄であることを確認
- [ ] 3部位患者（存在する場合）: BR21 に負傷原因テキストが入ることを確認
...
- [ ] B案実行 → xlsx を開いて **E56**（ユーザー表示行54）に当月最終日が入ること確認
- [ ] 書式: `令和X年Y月Z日` の文字列が入力されていること確認
- [ ] 日付が入らない場合: Cloud Run ログで `[D6-DATE]` 行を確認してください
