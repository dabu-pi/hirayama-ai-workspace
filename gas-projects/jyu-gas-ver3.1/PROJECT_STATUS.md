...
最終更新: 2026-04-02（**WS-SR プレースホルダー差分確認完了 — 4件追加・§6最終確定 / T-SR-01残作業4点のみ**）
...
**次の作業:** WS-SR — **T-SR-01 残作業: テンプレートに4点を追加入力（`{{被保険者生年月日}}` `{{日数1}}` `{{日数2}}` `{{施術回数2}}`）** → 完了後に T-SR-02〜 GAS実装。 / 改善候補 I-1 保留中。
...
**ステータス: プレースホルダー最終確定（2026-04-02更新） / T-SR-01 残4点のみ / 完了次第 GAS 実装へ**
...
| # | タスク | 種別 | 状態 |
|---|---|---|---|
| **T-SR-01** | **テンプレートへのプレースホルダー手入力（§6 最終確定版 30項目）** | **人間が実施** | **⏳ 進行中 — 残4点：`{{被保険者生年月日}}` `{{日数1}}` `{{日数2}}` `{{施術回数2}}`** |
...
1. **T-SR-01 残作業（人間が実施）:** テンプレートに以下4点を追加入力する
   - `{{被保険者生年月日}}` → 被保険者の生年月日欄
   - `{{日数1}}` → 負傷名1行目の「日数」欄
   - `{{日数2}}` → 負傷名2行目の「日数」欄
   - `{{施術回数2}}` → 負傷名2行目の「施術回数」欄
2. **T-SR-01 完了後:** 「T-SR-01 完了・GAS実装開始」で Claude Code に依頼 → T-SR-02〜 実装
