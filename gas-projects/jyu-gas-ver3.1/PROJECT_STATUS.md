...
| H | **施術録 表面 部位2 空行削除（1部位患者対応）** | Claude Code | **✅ 実装完了（2026-04-19）clasp push OK** |

**施術録 部位2空行削除 修正記録（2026-04-19）— Task H:**

| 項目 | 内容 |
|---|---|
| 症状 | 1部位患者の施術録表面に「部位2」行が空行として表示される |
| 根本原因 | `srInsertHyomenData_` は部位2がなくても常に `rep('負傷名2', '')` 等を実行していた。プレースホルダーは空文字に置換されるが、テンプレートの行自体は残る |
| 修正 | `hasCase2` を rep() 呼出前に評価。部位2データあり → 既存置換。部位2なし → `srRemoveHyomenPartRow_()` で `{{負傷名2}}` を含むテーブル行ごと削除 |
| 追加関数 | `srRemoveHyomenPartRow_(body, sentinelKey)` — body内の全テーブルをスキャンし `{{sentinelKey}}` を含む行を `table.removeRow()` で削除 |
| 安全設計 | ① sentinel が見つからない場合は WARN ログのみ（crash なし）② hasCase2=true のパスは既存ロジックを完全維持 |
