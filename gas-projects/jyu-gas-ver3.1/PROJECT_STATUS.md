...
|---|---|---|---|---|---|---|
| U1 | 都道府県番号 | ✅ **CI2**（CI2:CL3）| ❌ 未実装 | ❌ 未実装 | 設定シート固定値（施術機関所在都道府県、2桁） | 中 |
| U2 | 施術機関コード | ✅ **CZ2**（CZ2:DV3）| ❌ 未実装 | ❌ 未実装 | 設定シート固定値（国保連付番コード） | 中 |
| U3 | 保険種別 | ✅ CB8〜CJ11（○付）| ✅ `master.insuranceType` | ✅ `INSURANCE_TYPE_CELLS` で○付 | 保険者番号から自動判定 or 患者マスタ | **DONE** |
| U4 | 単併区分 | ✅ **CT8**（単独）/ CT10（2併）/ CT12（3併）| ❌ 未実装 | ❌ 未実装 | 固定「単独」→CT8 / 将来: 患者マスタ「公費区分」 | 中 |
| U5 | 本家区分 | ✅ **DB8**（本人）/ DB10（六歳）/ DB12（家族）/ DH8（高一）/ DH12（高7）| ⚠️ 続柄はあるが複合コード判定未実装 | ❌ | 続柄+保険者番号+年齢 → コード選択 | 中 |
| U6 | 給付割合 | ✅ **DP8**（10・9割）/ **DP11**（8・7割）| ✅ `V3TR_pickBurdenDigit_` で算出済み | ❌ セルへの書込なし | `10 − 一部負担金割合`（1→9→DP8, 2→8→DP11, 3→7→DP11）| 中 |
| U7 | 請求区分（新規・継続） | ✅ **DH31**（DH31:DV31）| ❌ `row["請求区分"]` 未生成 | ❌ CELL_MAP定義済みだがput()なし | 初検月=対象月→新規 / 前月以前→継続 / 同月内治癒再発→両方○ | **高** |
