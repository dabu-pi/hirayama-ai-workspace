...
- 状態: 稼働中（B案目視確認完了・Cloud Run 起動安定化完了。D2/U5後期高齢者対応確定済み。残課題: D5/U2/U6暫定運用欄公式確認）
- 優先度: 最優先
...
| 4 | U2 施術機関コード | ⚠️ **一部確認・値は要照合** | **公式根拠確定（2026-03-21）**: 施術機関コード = 国保連合会が付番したコード（神奈川県国保連記載要領原文）。**ただし現行値 `2804440-0-0` が兵庫県国保連施術機関コードと同一か未照合**。届出承諾通知書を確認して照合すること。 |
...
> 最終更新: 2026-03-21（U6 給付割合 制度確定完了）
> 次: U2 実値照合（オーナー対応待ち）/ 高7「⓪」実機確認（患者来院時）/ D4 3部位実案件確認（発生時）
