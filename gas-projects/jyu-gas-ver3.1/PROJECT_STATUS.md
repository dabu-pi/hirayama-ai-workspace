...
最終更新: 2026-04-04（**JREC-01 出力物/テンプレート分離: 月次出力共通化・旧版退避・設定シート優先化・Markdown正本整理**）
...
**次の作業:** 今回は live での生成成功と新フォルダ構造への出力整合まで確認済み。帳票内容の細部検証（個別セル内容、施術終了年月日など）は別スコープで実施する。`設定!施術録テンプレートID` 未設定時 fallback の確認と、申請書テンプレ `application_template.xlsx` の repo 物理移動は、必要なら別タスクで Cloud Build パス安全性を確認してから進める。
...
| 4. 次の作業候補 | **帳票内容の細部検証は別スコープ**。今回あえて見ていない個別セル内容・施術終了年月日などを確認する場合は、WS-SR の詳細検証タスクとして分けて実施する。次に必要なら WS-SR **請求年月日 / 領収年月日 表示実装** へ戻る。 |
...
| 項目 | 方針 |
|---|---|
| ①請求年月日 | **未実装のまま保留**。保険者へ当月分の請求書を提出した日を記録する |
| ①領収年月日 | **未実装のまま保留**。当月分の患者負担額を最終的に受領した日を代表記録として記録する |
| 月内に複数回の領収がある場合 | 施術録の領収年月日は最終受領日を記録し、各回の領収日は別の会計記録・領収証控え等で管理する |

**再開用メモ:**
- 今回は Markdown記録の更新のみ。コード変更・テンプレート変更・Dashboard反映・Run_Log実更新は行っていない。
- 将来再開する場合は、上記の運用方針と「請求期間セルは同一セル2行構造」という前提を維持したまま、**①請求年月日 / ①領収年月日 の表示実装** から着手する。
