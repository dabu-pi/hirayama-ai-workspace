...
### 保存後会計サマリー兼領収証導線 設計フェーズ（2026-03-31）

設計書: `docs/JREC-01_RECEIPT_SUMMARY_DESIGN_2026-03-31.md`
ステータス: **設計完了・院長の案選択待ち（実装未着手）**

#### 目的

保存完了後に「患者名・来院日・4項目の金額・visitKey」を一画面で確認できる「直前保存サマリー」を設ける。
将来の領収証印刷の参照元をこのサマリーに統一する。
...
| 案 | 場所 | 推奨タイミング |
|---|---|---|
| **案A（推奨・短期）** | **患者画面シート 行55〜57** | **プレオープン〜運用開始** |
| 案B（中長期）| 専用「会計サマリー」シート | 印刷需要が出たとき |
| 案C（コード変更最小）| 患者画面右端に参照式 | 暫定対応のみ |

#### 案A の実装概要（院長が選択した場合）

| 変更箇所 | 内容 |
|---|---|
| `Ver3_core.js` | `writeSavedSummaryUI_V3_` 新設 |
| `clearAfterSaveUI_V3_` 末尾 | サマリー書き込みを追加 |
| `clearEntryUI_V3` | サマリーエリアのクリアを追加 |
| `setupSelfPayValidation_V3_` | A55:H62 クリアを廃止 or 初回1回に変更 + サマリーラベル設置 |

#### 既存処理との切り分け

```
clearAfterSaveUI_V3_:
  入力欄クリア（維持）+ E2〜E6 残す（維持）+ サマリーを書く（追加）

clearEntryUI_V3（手動全クリア）:
  E2〜E6 クリア（維持）+ サマリーもクリア（追加）
```

#### 未確認事項（院長が実機確認後に着手）

1. 行55〜62 の現状（空欄か残存データあるか）
2. `clearEntryUI_V3` が行55〜62 を対象としているか
3. `setupSelfPayValidation_V3_` の A55:H62 クリア廃止の可否
4. 患者名の取得元（患者マスタ参照 or UI B3 の表示文字列）
