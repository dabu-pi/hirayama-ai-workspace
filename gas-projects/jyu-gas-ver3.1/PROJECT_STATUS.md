...
最終更新: 2026-04-06（**再読み込み時の経過表示仕様修正 clasp push 完了・実機確認待ち**）

---

## 🆕 2026-04-06 再読み込み時の経過表示 仕様修正（commit `3a18573`）

**問題:** 再読み込み後、「経過（今回）」が空白 / 経過履歴に対象日当日・以降が混入していた。

| 修正内容 | 詳細 |
|---|---|
| `findCaseRowByPatientDateCaseNo_` | return に `keikaNow` フィールドを追加 |
| `buildKeikaHistoryTextFromCases_` | `cutoffDate` 引数を追加（指定時は当日・以降を除外） |
| `reloadVisitToUI_V3` | keikaNow を「経過（今回）」へセット + cutoff 付き履歴をセット |

**修正後の UI 仕様（例: 3/3 再読み込み時）:**
- 経過（今回） = 3/3 の `keikaNow`
- 経過履歴 = 3/2 以前のみ（最新5件）

**clasp push:** 完了（2026-04-06）
**次アクション:** 「当日内容を再読み込み」で経過（今回）が表示されるか実機確認。経過履歴に当日が混ざらないか確認。

**詳細:** `docs/JREC-01_reload_and_outcome_2026-04-06.md`（⚠️ 仕様修正セクション参照）

---

## 🆕 2026-04-06 経過後修正機能 実装完了

| 機能 | メニュー | 状態 |
|---|---|---|
| 経過を更新 | 柔整管理 > 経過を更新 | 実装完了・実機確認待ち |

**追加関数:** `updateProgressFromUI_V3` / `collectProgressPatchFromUI_`

**更新対象:** `CASE_COLS.keikaNow`（"経過_今回"）のみ。転帰・終了日・金額・治療法フラグ・所見は変更しない。

**現場フロー:** 再読み込み → 経過欄修正 → 経過を更新 → 再読み込みで反映確認

**詳細:** `docs/JREC-01_update_progress_2026-04-06.md`

---

## ✅ 2026-04-06 過去日再読み込み・転帰後付け更新 — 実機確認完了

| 機能 | メニュー | 実機確認 |
|---|---|---|
| 当日内容を再読み込み | 柔整管理 > 当日内容を再読み込み | ✅ OK（初版）/ 🔄 経過表示修正後・実機確認待ち |
| 転帰を更新 | 柔整管理 > 転帰を更新 | ✅ OK |

**追加関数:** `reloadVisitToUI_V3` / `updateOutcomeFromUI_V3` / `writeExactCaseSrcToUI_` / `readCaseKubun_`

**安全設計:** 金額・初検/再検区分・治療法フラグは変更しない。転帰と終了日の2項目のみ後付け書き込み。

**現場フロー:** B2患者選択 → B4過去日入力 → 再読み込み → H列転帰/G列終了日入力 → 転帰を更新
