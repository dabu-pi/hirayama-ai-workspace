...
最終更新: 2026-03-20（共有復旧後 mineo 患者マスタ直接確認・制度根拠記録完了）
...
| 未確認内容 | 理由 | 次アクション |
|---|---|---|
| B案プリフライト warning W1 が出ないこと | mineo の `負担割合=0.1` は正しいが実行確認がまだ | 【手順①】B案実行 → mineo で warning が出ないことを確認（患者マスタ修正は不要） |
...
| 優先 | タスク | 理由 |
|---|---|---|
| 1 | **【人間作業】B案実行 → mineo warning W1 なし・全員生成を確認** | `負担割合=0.1` は確認済み（修正不要）。B案実行でプリフライトクリーン + 3件生成 + P1出力不変を一括確認 |
...
| 5 | 高7 の "⓪" 表示確認 | Unicode U+24EA が帳票上で正しく表示されるか未確認 |
| 6 | D2 継続月数・頻回 | 制度定義（連続月数・月10回以上の連続月数）の再確認後に実装 |
| 7 | U2 施術機関コードの公式確認 | 暫定ルール（先頭協/契除去・ハイフン保持）の公式根拠が未確認 |
