...
最終更新: 2026-04-02（**WS-SR Phase 1 実装設計確定 — T-SR-01（人間：プレースホルダー手入力）完了後にGAS実装へ**）
...
**次の作業:** WS-SR — **T-SR-01（人間）: テンプレートにプレースホルダーを手入力** → T-SR-02〜 GAS実装（`Ver3_shuRecorder.js`）。 / 改善候補 I-1（来院ヘッダ列順整理）は保留中。
...
**ステータス: Phase 1 実装設計確定（2026-04-02更新） / sandbox 実機確認完了 / 実装着手可（T-SR-01 が先行条件）**
...
| # | タスク | 種別 | 状態 |
|---|---|---|---|
| **T-SR-01** | **テンプレートへのプレースホルダー手入力（§6 一覧参照）** | **人間が実施** | **⏳ 未実施（先行条件）** |
| T-SR-02 | `Ver3_shuRecorder.js` 新規作成・設定定数定義 | GAS実装 | 未着手 |
| T-SR-03 | データ取得関数（患者マスタ / 来院ケース / 来院ヘッダ+明細）| GAS実装 | 未着手 |
| T-SR-04 | `srGetInitExamData_` 実装（初検情報履歴） | GAS実装 | 未着手 |
| T-SR-05 | テンプレート複製 + 出力フォルダ管理 | GAS実装 | 未着手 |
| T-SR-06 | `srInsertHyomenData_`（replaceText 全欄） | GAS実装 | 未着手 |
| T-SR-07 | `srInsertUrameData_`（日別行 + ①集計） | GAS実装 | 未着手 |
| T-SR-08 | `srExportPdf_` | GAS実装 | 未着手 |
| T-SR-09 | ダイアログ表示 + 再実行チェック + メニュー登録 | GAS実装 | 未着手 |
| T-SR-10 | clasp push + 手動テスト（1患者・2026-04） | 実機確認 | 未着手 |
| T-SR-11 | テスト結果を TESTCASES.md に記録 | ドキュメント | 未着手 |

### 次のアクション（WS-SR）

1. **T-SR-01（人間が実施）:** テンプレート（ID: `1Tcq8kcwFfIzFixGF54xFoWyZcNz7IsgjYsT8NqV0mnY`）を開き、§6 プレースホルダー一覧に従って各セルへ `{{KEY}}` を手入力する
2. **T-SR-01 完了後:** Claude Code で T-SR-02〜 GAS 実装（`Ver3_shuRecorder.js`）を依頼する
