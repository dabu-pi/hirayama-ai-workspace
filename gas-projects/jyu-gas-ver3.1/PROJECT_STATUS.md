...
| **出力物/テンプレート分離** | **共通月次出力 `JREC-01_月次出力/YYYY-MM/{01_申請書,02_施術録,90_再生成旧版}` / `Ver3_outputManager.js` 追加 / 施術録 Template ID 設定シート優先 / 分離ルール docs 正本化** | **✅ repo整理完了・live目視確認待ち（2026-04-04）** |

**次の作業:** `clasp push` 後、申請書 B案と施術録 WS-SR を 1患者・1月で再生成し、Drive に `JREC-01_月次出力/YYYY-MM/{01_申請書,02_施術録,90_再生成旧版}` が作られること、旧版がトラッシュではなく退避されること、`設定!施術録テンプレートID` 未設定時 fallback が効くことを目視確認する。申請書テンプレ `application_template.xlsx` の repo 物理移動は Cloud Build パス安全性を別途確認してから実施する。
...
| 3. 現状 | **出力物/テンプレート分離の repo 正本整理完了。申請書/施術録の保存先を共通月次フォルダへ寄せ、旧版は `90_再生成旧版` 退避、施術録テンプレIDは設定シート優先に変更。運用ルールは `docs/JREC-01_出力物テンプレート管理.md` と `docs/JREC-01_TEMPLATE_INDEX.md` に記録済み。** |
| 4. 次の作業候補 | **live 目視確認**: `clasp push` → 申請書 B案 / 施術録 WS-SR を1患者で再生成 → 新フォルダ構造と旧版退避を確認。次に必要なら WS-SR **請求年月日 / 領収年月日 表示実装** へ戻る。 |
