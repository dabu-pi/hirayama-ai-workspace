...
最終更新: 2026-04-04（**WS-SR v13 live未反映原因切り分け: 請求年月日 / 領収年月日**）
...
| **WS-SR 個別セル内容ルール** | **施術終了年月日=転帰なし空欄/転帰あり実日付、①請求年月日=対象月末日（暫定運用）、①領収年月日=当月最終来院日。v13 で marker なしテンプレでも請求年月日/領収年月日セルを書けるよう補助探索を追加** | **✅ 実装完了・Node確認PASS・`clasp push --force` 反映済み・`clasp run` 権限エラーのため live 1患者再生成目視は手動確認待ち（2026-04-04）** |

**次の作業:** `clasp push --force` は完了済み。スプレッドシート UI または Apps Script エディタから 1患者1月の施術録を再生成し、`施術終了年月日 / ①請求年月日 / ①領収年月日` の値と配置を live 目視確認する。請求年月日は現時点で **対象月末日の暫定運用** なので、公式ルールが固まったら差し替える。`設定!施術録テンプレートID` 未設定時 fallback の確認と、申請書テンプレ `application_template.xlsx` の repo 物理移動は別タスクで進める。
...
| 4. 次の作業候補 | スプレッドシート UI または Apps Script エディタから 1患者1月の施術録を再生成し、**施術終了年月日は転帰なし空欄/転帰あり実日付、①請求年月日は対象月末日（暫定）、①領収年月日は当月最終来院日** になっているか live 目視確認する。`clasp run srRunTsr10v2Debug_` は現環境で権限エラーのため、再生成確認は手動実行前提。 |
...
| live 1患者1月再生成の最終目視 | **次確認事項**。`clasp push` 後に `施術終了年月日 / 請求年月日 / 領収年月日` の表示位置と値を確認する |
...
| V13-6 | `clasp push --force` で修正版を再反映。`clasp run srRunTsr10v2Debug_` は `Unable to run script function. Please make sure you have permission to run the script function.` のため、1患者1月の live 再生成目視は手動確認へ持ち越し | ⚠️ 手動目視待ち |
...
| 確認済み | 最新コードの bound script push、テンプレート現物構造、ローカル構文チェック、marker なし表の座標解決、請求年月日=月末日 / 領収年月日=最終来院日の値生成 |
| 今回の未完了 | UI/Apps Script エディタからの 1患者1月 live 再生成目視。`clasp run` が現環境で権限エラーのため、自動実行確認はできていない |
