...
最終更新: 2026-04-04（**WS-SR 個別セル内容ルール反映: 施術終了年月日 / 請求年月日 / 領収年月日**）
...
| **WS-SR 個別セル内容ルール** | **施術終了年月日=転帰なし空欄/転帰あり実日付、①請求年月日=対象月末日（暫定運用）、①領収年月日=当月最終来院日** | **✅ 実装完了・Node確認PASS・live 1患者再生成目視は次確認（2026-04-04）** |

**次の作業:** `clasp push` 後、1患者1月で施術録を再生成し、`施術終了年月日 / ①請求年月日 / ①領収年月日` の値と配置を live 目視確認する。請求年月日は現時点で **対象月末日の暫定運用** なので、公式ルールが固まったら差し替える。`設定!施術録テンプレートID` 未設定時 fallback の確認と、申請書テンプレ `application_template.xlsx` の repo 物理移動は別タスクで進める。
...
| 4. 次の作業候補 | `clasp push` → 1患者1月の施術録を再生成し、**施術終了年月日は転帰なし空欄/転帰あり実日付、①請求年月日は対象月末日（暫定）、①領収年月日は当月最終来院日** になっているか live 目視確認する。 |
