...
|---|---|---|---|---|---|---|
| U1 | 都道府県番号 | 未確認（上段左）| ❌ 未実装 | ❌ 未実装 | 設定シート（固定値） | **高** |
| U2 | 施術機関コード | 未確認（上段左）| ❌ 未実装 | ❌ 未実装 | 設定シート（固定値） | **高** |
| U3 | 保険種別 | CB8〜CJ11（○付） | ✅ `master.insuranceType` | ✅ `INSURANCE_TYPE_CELLS` で○付 | 保険者番号から自動判定 or 患者マスタ | **DONE** |
| U4 | 単併区分 | 未確認 | ❌ 未実装 | ❌ 未実装 | 固定（柔整は基本「単独」）か設定値 | **中** |
| U5 | 本家区分 | 未確認 | ⚠️ `続柄` はtransferColsにあるが申請書セルへの書込なし | ❌ 未実装 | `続柄` から判定（本人=本人、それ以外=家族） | **中** |
| U6 | 給付割合 | 未確認（`一部負担金割合` DP45と別） | ✅ `V3TR_pickBurdenDigit_` で算出 | ❌ 給付割合セルへの書込なし | `一部負担金割合`（3/2/1割）から逆算 | **中** |
| U7 | 請求区分（新規・継続） | DH31 | ❌ `row["請求区分"]` 未定義 | ❌ `CELL_MAP["請求区分"]` は定義済みだが `put()` なし | 初検年月日 = 対象月なら「新規」、それ以外「継続」| **高** |

#### 各欄の詳細

**U1: 都道府県番号**
- テンプレートの行1-3あたりに存在するはずだが、セル番地未確認（openpyxlスキャン未実施）
- 固定値（施術機関の都道府県）→ 設定シートで一元管理する方針
- GAS: `transferCols` 追加 + 設定シートから取得 / Python: `CELL_MAP` 追加 + `put()` 追加

**U2: 施術機関コード**
- テンプレートの行1-3あたり、都道府県番号の隣に存在するはずだが、セル番地未確認
- 固定値（施術機関の登録コード）→ 設定シートで一元管理する方針

**U4: 単併区分**
- 単独施術か他制度との併給かを示す欄
- 柔整保険請求では「単独」が基本のため、固定値「単独」で対応できる可能性が高い
- 制度上の判定基準と様式上の記載要領を要確認

**U5: 本家区分（本人・家族）**
- `続柄` = "本人" → 「本人」 / それ以外 → 「家族」 と判定可能
- `続柄` は `transferCols` に存在するが、申請書セルへの書込処理がない
- セル番地が未確認（テンプレートのスキャン要）

**U6: 給付割合**
- 「一部負担金割合」（3/2/1割 = 30%/20%/10%負担）から逆算して7/8/9割給付
- GAS側: `V3TR_pickBurdenDigit_` が `一部負担金割合` を返す（1=1割負担=9割給付）
- Python側: `put_amount_digits(AMOUNT_DIGIT_CELLS["一部負担金"], ...)` で一部負担金金額を書くが、**給付割合（割合数値）のセルへの書込がない**
- セル番地の確認と書込処理の追加が必要

**U7: 請求区分（新規・継続）**
- DH31（マージセル DH31:DV31）への書込が必要
- `CELL_MAP["請求区分"] = "DH31"` は定義済みだが、`put(CELL_MAP["請求区分"], ...)` の呼び出しがない
- `row["請求区分"]` も GAS 側で未生成（`transferCols` に未定義）
- **判定ロジック**: 初検年月日の月 = 対象月 → 「新規」 / 初検年月日の月 < 対象月 → 「継続」
- 最優先で実装すべき欄（毎月必ず書く・判定ロジックが単純）
