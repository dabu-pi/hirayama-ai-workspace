...
最終更新: 2026-04-03（**WS-SR 表示追加修正 v10: 請求期間年付き略記・円表記・転帰既定表示**）
...
| **WS-SR 表示追加修正** | **施術終了年月日プレースホルダー / earliest case / 裏面2件目セクション / 裏面①集計ブロック項目別配置 / ①請求期間 同一セル2行表示 / 金額円表記 / 転帰既定表示** | **✅ 実装完了・実機確認待ち（2026-04-03）** |

**次の作業:** WS-SR — `clasp push` → `srGenerateDocument('P001', '2026-03')` 手動実行 → **裏面①集計ブロック v10 と表面転帰欄の目視確認**。T-SR-17 / T-SR-18 と施療料/後療料の副作用も併せて確認。改善候補 I-1 保留中。
...
| 3. 現状 | **WS-SR 表示追加修正 v10 実装完了・実機確認待ち（請求期間年付き略記・円表記・転帰既定表示）** |
| 4. 次の作業候補 | `clasp push` → `srGenerateDocument('P001', '2026-03')` → `[INFO] ①請求期間 表示値 from=R8.3.1 to=R8.3.9` / `[INFO] ①合計金額 表示値=...円` / `[INFO] 転帰 ...` を確認 → 現物目視確認 |
