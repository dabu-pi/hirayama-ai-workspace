...
最終更新: 2026-04-02（**WS-SR 施術録様式確認完了 — 1患者×月出力・Word方式を現時点方針として確定**）
...
**次の作業:** WS-SR — JREC来院ヘッダ列確認（施術経過所見欄の有無）→ GAS + Docs API 技術検証 → 実装フェーズ設計。 / 改善候補 I-1（来院ヘッダ列順整理）は保留中。
...
**ステータス: 様式確認完了・設計方針確定（2026-04-02更新） / 実装未着手**
...
| # | 論点 | 状況 |
|---|---|---|
| ① | JRECに「施術経過所見」入力欄があるか | **要確認**（来院ヘッダ列を確認）|
| ② | 冷罨法・電療・明細書加算の施術明細実装状況 | **要確認** |
| ③ | GAS → docx/Docs API 操作の技術可否 | **要確認**（sandbox検証）|
| ④ | Archive化のタイミングと Drive フォルダ設計 | 未確定 |
| ⑤ | 自費混在ケースの扱い | 初版スコープ外 |

### 次のアクション（WS-SR）

1. JREC来院ヘッダの列構成を確認（施術経過所見列の有無）
2. 施術明細の列構成を確認（冷罨法・電療等）
3. GAS + Docs API の技術検証（sandbox）
4. 実装フェーズ設計（Phase C or WS-SR Phase 1）
5. 実装着手
