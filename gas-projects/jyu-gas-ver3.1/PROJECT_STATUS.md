...
最終更新: 2026-04-02（**WS-SR 転記元調査完了 — 主要列は全て既存で揃い・GAS+Docs API 技術検証が次**）
...
**次の作業:** WS-SR — GAS + Google Docs API 技術検証（sandbox で docx テンプレ複製→値差し込み）→ 実装フェーズ設計。 / 改善候補 I-1（来院ヘッダ列順整理）は保留中。
...
**ステータス: 転記元調査完了・設計方針確定（2026-04-02更新） / 実装未着手**
...
| # | 論点 | 状況 |
|---|---|---|
| ① | GAS → Google Docs API の技術可否 | **要検証**（sandbox）|
...
1. **GAS + Google Docs API 技術検証**（docxテンプレ複製→値差し込み）← sandbox で実施
2. 整復料/後療料の列振り分けロジック確定
3. `shoken` / `keikaNow` 主転記列確定
4. 実装フェーズ設計（WS-SR Phase 1）
5. 実装着手
...
- **転記元対応表: `docs/施術録導線/JREC-01_施術録転記元対応表.md`** ← 今回作成
