...
- 状態: 稼働中（B案最終確認完了・3件生成/エラー0件。残課題: D2/D5/U5後期高齢者/暫定運用欄）
- 優先度: 最優先
...
**B案（申請書生成）は完全に動作確認済み。** 次の作業は制度定義調査（D2）または暫定運用欄の公式確認（U2/U5/U6）。

| カテゴリ | 状態 | 詳細 |
|---|---|---|
| B案申請書生成 | ✅ **本番稼働可能** | 3件生成/エラー0件/全患者クリーン（2026-03-20確認済み）|
| 申請書上段欄 U1〜U7 | ✅ 全欄実装済み | U2/U5/U6 は暫定運用（制度公式確認が残課題）|
| D2 継続月数・頻回 | ⚠️ 保留中 | 制度定義（月10回以上の連続月数）の再確認が必要 |
...
| 優先 | タスク | 種別 | 理由 |
|---|---|---|---|
| 1 | D2 継続月数・頻回 | 保留（制度定義再確認待ち）| 「月10回以上の連続月数」の正確な定義確認が必要。確認後に実装 |
