...
> 最終更新: 2026-03-17（fixture 21/21 PASS 確認・第一段階完了）

### ✅ fixture テスト基盤 第一段階 完了（2026-03-17）

- 21/21 PASS（TC01〜TC10 サブケース + M01〜M05）
- koryoDakkyu=720 確認・TC08 偽陽性解消済み
...
| 優先 | 項目 | 内容 |
|---|---|---|
| 高 | seifukuDakkyu 確認 | TEST_SETTINGS_.seifukuDakkyu=0（TODO）。設定シートで接骨料_脱臼の実値を確認してフィクスチャに反映 |
| 中 | `exportHeaderFromCases_V3` 新5列配線 | ヘッダ再生成時に算定区分/Mixed区分/case要約/課金理由の5列も出力されるよう配線 |
| 中 | `transferData` 新5列反映 | 月次転記・申請書データへの新5列反映可否を検討 |
| 低 | 既存データ一括再計算メニュー | 過去来院ヘッダに新5列を遡及反映するメニュー |
| 低 | TC09 再検月上限ロジック追加 | 現在はcaseKey単位管理のみ。患者月単位の再検1回制限を実装するか否か要判断 |

### 中長期

- 算定ロジックの回帰確認（TC01〜TC10 通過確認）
- 申請書生成フローの実運用確認
