...
## 📋 2026-04-26 初検時情報・所見・経過欄 運用整理

設計メモ: `docs/JREC-01_initinfo_shoken_keika_investigation_2026-04-26.md`

### 背景

UI シートの「初見ケース1のように見える欄（A23:B28）」と「初検時情報欄（E16:G20）」が重複していないかを調査した。

### 各エリアの役割まとめ

| セル範囲 | コード定数 | 役割 | 保存先 | 帳票出力先 |
|---|---|---|---|---|
| E16:G20 | `case1_initInfo` | 初検時情報（負傷日時・場所・状況・所見・相談支援） | 初検情報履歴シート | 施術録 **表面** / 申請書 |
| A23:B28 | `case1_shoken` | 所見（毎来院の臨床観察メモ） | CASES シート（所見列） | 施術録 **裏面** |
| A16:B20 | `case1_keikaNow` | 今回の経過（毎来院の経過メモ） | CASES シート（経過_今回列） | 経過履歴表示の源泉 |
| D23:G28 | `case1_keikaHistory` | 経過履歴表示（表示専用・最大5件） | 保存なし（表示のみ） | — |

### 統合非推奨の理由

- E19(initFindings) と A23:B28(shoken) は初回来院日に実入力内容が重複しやすい
- ただし帳票上は別の場所に出力される（E19 → 表面/申請書、shoken → 裏面）
- 片方を廃止するとその帳票が空欄になる
- **完全統合・廃止は非推奨**

### A23:B28 の引継ぎ挙動（重要）

`autofillFromPreviousVisit_V3` は「エピソード開始日（初検日）の所見」を引き継ぐ設計。  
→ 2日目以降も「初検日の所見」が A23:B28 に表示されるため「初回のみの欄」に見えるが、毎回保存されている。
...
| 案 | 内容 | 難易度 |
|---|---|---|
| **ラベル変更** | A23:B28 の隣ラベルを「所見（施術録裏面）」「来院時所見」などに変更 | 最低（手動のみ） |
| **コピーメニュー追加** | 「初検時所見 → 本日の所見へコピー」メニュー関数を追加 | 低 |
| **経過履歴件数上限変更** | `buildKeikaHistoryTextFromCases_` の引数 `5` → `8` などへ | 低（3行変更のみ） |
| **初検時自動コピー** | `saveVisit_V3` 初検時に空なら initFindings → shoken へコピー | 中（saveVisit_V3 変更必要） |
| シート物理拡張 | 行追加・ケース2セル参照全更新 | 高（大規模変更） |

### 付記: SPEC.md の旧記述

SPEC.md の「部位名欄（A12, A13, A27, A28）」「B27, B28」は旧レイアウト時代の記述。  
現行 case2 は row36/37。次回 SPEC.md 更新時に修正すること。

---

## 📋 2026-04-26 Phase A-1 SC1 設計課題記録

### 経緯

Phase A-1 実受付テスト SC1「前回自費明細がある患者で前回明細が反映される」を実施したところ、
「前回来院（04/22）に自費明細がありません。通常の明細入力を出します」が表示された。

実データ上は **04/23 に自費明細データが存在**しており、日付ずれではなく設計ギャップと判断した。

### 根本原因（コード調査で確認済み）

**来院ヘッダ未作成のまま自費明細が保存できる設計ギャップ**

| 確認事項 | 結果 |
|---|---|
| 来院ヘッダを作る唯一の経路 | `saveVisit_V3` のみ |
| Web UI 患者選択時のヘッダ作成 | なし（`setPatientAndDate_V3` は B2/B4 書き込みのみ） |
| `saveSelfPayDetailsFromDialog_V3` のヘッダ有無チェック | なし（ヘッダ未作成でも自費明細保存可） |
| `loadPrevSelfPayToDialog_V3` の前回来院日検索先 | 来院ヘッダのみ |

**失敗の経路:**
```
04/23: 来院ヘッダ未作成のまま自費明細のみ保存
  → 自費明細シート: patientId_2026-04-23 ✓
  → 来院ヘッダ: 04/23 行なし

04/26: loadPrevSelfPayToDialog_V3 実行
  → findLastVisitDateInHeader_: ヘッダ最新 = 04/22
  → prevVisitKey = "patientId_2026-04-22"
  → 04/22 の自費明細なし → アラート
```
...
- upsert 化は影響範囲が大きく、Phase A の実受付テスト評価がぶれる
- 来院ヘッダの自動作成は二重記録禁止ポリシーと密接に関係する

テスト記録: `docs/JREC-01_phaseA_operational_test_2026-04-23.md` §8-C を参照

### Phase B 候補として記録

**「本日の来院ラインを確定する」明示操作方式を採用候補とする。**

| 設計方針 | 内容 |
|---|---|
| 来院ライン確定 | patientId + treatDate から visitKey を一度だけ確定 |
| 冪等保証 | 同一 visitKey が来院ヘッダに存在する場合は新規作成しない・既存を再利用 |
| 二重記録禁止 | 同一 patientId + 同日 の来院ラインは 1 本のみ |
| 可視化 | 確定前: 「未確定」/ 確定後: 「来院確定済み・visitKey・確定日時」を表示 |
| 自費明細は確定後に | 来院ライン確定後に自費明細・施術記録・会計へ進む |
| saveVisit_V3 変更 | throw → upsert 化（Phase B の最優先・最高リスクタスク） |

UI設計案: `docs/JREC-01_single_page_ui_design_2026-04-25.md` §8 を参照

### SC1 現在のステータス

`docs/JREC-01_phaseA_operational_test_2026-04-23.md` §8-B の SC1 を
「未確認」→「**設計課題により保留**」→「**暫定対応済み・実受付再確認待ち**」に更新済み。

**暫定対応内容:** `loadPrevSelfPayToDialog_V3` にフォールバック追加（§2026-04-26 Phase A-1 暫定対応セクション参照）
...
## 📋 2026-04-26 Phase A-1 SC1 暫定対応 — 前回自費明細フォールバック追加

### 変更内容

| ファイル | 変更 |
|---|---|
| `Ver3_core.js` | 補助関数 `findLatestSelfPayVisitKeyBeforeDate_` 追加（新規） |
| `Ver3_core.js` | `loadPrevSelfPayToDialog_V3` にフォールバック分岐追加 |
...
```
【従来】
  来院ヘッダ → 前回日 04/22 → 自費明細0件 → アラート → 通常入力

【変更後】
  来院ヘッダ → 前回日 04/22 → 自費明細0件
    ↓ フォールバック発動
  自費明細シート直接検索 → patientId 一致 + treatDate 未満
    → 最新 visitKey 発見（例: pid_2026-04-23）
    → 04/23 の自費明細を読み込み → ダイアログに反映
    フォールバック使用時: Logger "[SC1 FALLBACK]" を出力
```
...
| テストケース | 判定 |
|---|---|
| SC1相当: ヘッダに 04/23 なし・自費明細に 04/23 あり → フォールバックで 04/23 を取得 | ✅ PASS（コードレビュー） |
| SC2相当: 前回自費明細がない患者 → フォールバックも null → 通常入力（エラーなし） | ✅ PASS（コードレビュー） |
| 既存正常系: ヘッダに前回来院あり + 自費明細あり → フォールバック発動せず | ✅ PASS（コードレビュー） |
| 誤取得防止: 他患者・当日・未来日・不正 visitKey をスキップ | ✅ PASS（コードレビュー） |

**実受付での最終確認は次回受付日に実施する。**

---

## 📋 2026-04-25 前回施術再読込 — 実装方針確定

調査結果: `docs/JREC-01_prev_selfpay_reload_investigation_2026-04-25.md`  
設計メモ: `docs/JREC-01_prev_selfpay_reload_design_2026-04-23.md`

### 背景

Phase A 実運用テスト中に「前回の施術内容を再読み込みして今回入力のベースにする機能」が現場で先に必要と判断。

### 既存との差分（設計調査で確認済み）

| 対象 | 既存機能 | 今回の空白 |
|---|---|---|
| 部位・傷病・受傷日・治療法 | `autofillFromPreviousVisit_V3`（実装済み） | — |
| **自費明細（メニュー・数量）** | **未実装** | **← ここを新規実装する** |

### 最小安全仕様（確定）

- 「前回」= 来院ヘッダから同一患者の直前来院日（`findLastVisitDateInHeader_` 使用）
- 引き継ぎ対象: `menuId` / `menuName` / `qty` のみ
- 単価・小計・フラグ類は引き継がない（マスタ取り直し・今回入力）
- 上書き事故防止: 今回の明細に既存データがある場合は確認ダイアログ
- 保存は自動実行しない（ダイアログを開くだけ。人間が確認後に保存）

### 未解決論点 → 解消済み（2026-04-25）
...
| SC | 内容 | 状態 |
|---|---|---|
| SC3 | PropertiesService 取得後クリア | ✅ PASS（コードレビュー） |
| SC4 | 通常「自費明細入力」が壊れていない | ✅ PASS（コードレビュー） |
| SC5 | 保存処理が従来どおり動く | ✅ PASS（コードレビュー） |
| SC1 | 前回自費明細がある患者で反映される | ✅ **PASS（実受付確認 2026-04-26）** e630283 フォールバック有効 |
| SC2 | 前回自費明細がない患者で通常ダイアログが開く | ✅ **PASS（実受付確認 2026-04-26）** 通常入力として動作確認 |

### SC2 実受付テスト記録欄

| 確認項目 | 結果 | メモ |
|---|---|---|
| 通常入力画面として開くか | ✅ PASS | 実受付確認済み |
| 入力できるか | ✅ PASS | 実受付確認済み |
| 保存できるか | ✅ PASS | 実受付確認済み |
| 「検索に戻る」で患者検索に戻れるか | ⬜ 詳細未確認 | 既存修正 ec86b02 で対応済み。次回受付で再確認予定 |
| 判定 | ✅ **PASS** | 実受付確認 2026-04-26 |
| 実施日 | 2026-04-26 | |
| 備考 | — | 詳細は `docs/JREC-01_phaseA_operational_test_2026-04-23.md` §8-D に記録 |

### 次のアクション

1. 2026-04-30 頃に GAS 実行ログ確認（エラー率・`[SC1 FALLBACK]` ログの有無）
2. 2026-05-07 目安で Phase A 完了判定 → Phase B 移行可否を判断
3. Phase B で来院ライン確定ボタン方式（`ensureVisitHeaderForPatientDate_V3`）を実装予定

---

## 📋 Phase A 実運用テスト — 実施中（2026-04-23〜）

テスト記録ドキュメント: `docs/JREC-01_phaseA_operational_test_2026-04-23.md`

### 現在地

| Phase | 内容 | 状態 |
|---|---|---|
| 実装完了 | P01 患者検索 + P04 自費明細（実機確認 PASS 済み） | **完了** |
| **Phase A** | 実運用テスト（コード変更なし）1〜2週間 | **▶ 実施中（2026-04-23〜）重大バグなし** |
| Phase B | P05 会計確認ページ（読み取りのみ） | 設計完了・未実装 |
| Phase C | P02 来院入力フォーム（下書きのみ） | 設計完了・未実装 |
| Phase D | P02 確定保存（`saveVisit_V3` 連動） | 設計完了・未実装（慎重に） |

### 試用ログ（概要）

| 日付 | 結果 | 備考 |
|---|---|---|
| 2026-04-23 | PASS | 初回実受付通過。シートを開かずに自費入力完結 |
| 2026-04-25 | PASS / OBSERVE | 重大バグなし。設計調査・Phase A-1 実装を並行実施。実受付直接試用は次回確認 |
| 2026-04-25 | SC3/SC4/SC5 PASS | Phase A-1 実機テスト（コードレビュー）。SC1/SC2 は実受付テスト待ち。実運用開始可能と判断 |

### 次のアクション

**引き続き実受付での試用と記録。コード変更不要。**

1. 問題・気づきを `docs/JREC-01_phaseA_operational_test_2026-04-23.md` §8 へ日々追記する
2. 目安 **2026-05-07** に §6「完了判定基準」を確認し、Phase B 移行を判断する
3. GAS 実行ログ（エラー率）を確認する（Phase A 開始から1週間後メド）

### Phase A 完了判定基準（要約）

- 重大バグ（業務継続不可）がゼロ
- 「シートを開かずに自費入力できた」を確認できる
...
| 方針 | 内容 |
|---|---|
| 計算・制度判定はGAS側に残す | 区分判定・金額計算・月上限制御をWebに持たせない |
| 保存経路は既存のまま | `saveVisit_V3` / `saveSelfPayDetailsFromDialog_V3` を経由する |
| 既存シートUIと並行運用 | Web UI が動かなくてもシート操作で業務継続できること |

---

## 📋 2026-04-22 来院入力〜会計Web化 — 設計フェーズ

設計書: `docs/JREC-01_visit_webui_design_2026-04-22.md`
...
| Phase | 内容 | 状態 |
|---|---|---|
| **現在地** | P01 患者検索 + P04 自費明細 実装済み | **完了** |
| **Phase A** | 実運用テスト（コード変更なし）1〜2週間 | **→ 2026-04-23 開始** |
| Phase B | P05 会計確認ページ（読み取りのみ） | 設計完了・未実装 |
| Phase C | P02 来院入力フォーム（下書きのみ） | 設計完了・未実装 |
| Phase D | P02 確定保存（`saveVisit_V3` 連動） | 設計完了・未実装（慎重に） |

### Webが判断してはいけない項目（設計原則）

区分（初検/再検/後療）・月上限・多部位逓減・長期減額・保険請求額・窓口負担割合 — これらは常にGASが唯一の判断者。

### 次のアクション（2026-04-22 時点記録）

Phase A（実運用テスト）を開始する。コード変更不要。  
患者検索+自費明細WebUIを実受付業務で1〜2週間試用する。  
確認項目は設計書 §5-2 を参照。

---

## ✅ 2026-04-22 Web UI 段階移行フェーズ — 実機確認済み・クローズ

### 確認済み範囲

| 機能 | 実機確認 | 備考 |
|---|---|---|
| 患者検索（キーワード入力 → カード表示） | **PASS** | 部分一致 / 最大20件 |
| 患者選択 → B2/B4 シート反映 | **PASS** | `setPatientAndDate_V3` / 当日日付自動セット |
| 自費明細 Web UI 表示 | **PASS** | `?page=selfpay&visitKey=xxx` で正常起動 |
| 既存明細の読み込み | **PASS** | `getSelfPayDataByVisitKey_V3` 経由 |
| 自費明細の保存 | **PASS** | D7/F7/H8 更新確認済み |
| 患者検索 → 自費明細 導線遷移 | **PASS** | `/exec?page=selfpay&visitKey=...` で正常遷移 |

### 到達点まとめ

スプレッドシートUIを開かずに、スマートフォン / ブラウザから以下の業務フローを完結できるようになった:

```
Web App URL を開く
  → 患者をキーワード検索
  → 患者カードをタップ（シート B2/B4 を自動更新）
  → 「自費明細入力 →」ボタンで自費明細ページへ遷移
  → メニュー選択・数量入力・保存
  → 「N 件の自費明細を保存しました」完了表示
```

既存の保存ロジック（`saveSelfPayDetails_V3_` / D7/F7/H8 更新）をそのまま流用しており、
シート構造・帳票出力・集計への影響はない。
既存スプレッドシートメニューからの操作も引き続き利用可能。

### 技術的ポイント（次フェーズへの申し送り）

| 知見 | 内容 |
|---|---|
| `window.location.href` は使わない | GAS Web App 内では `userCodeAppPanel` を返すため無効。`ScriptApp.getService().getUrl()` をテンプレート埋め込みで使う |
| `show()` に `display:""` を使わない | CSS ルールが残るため `"block"` 等を明示する |
| `createTemplateFromFile` でサーバー変数を渡す | visitKey / appBaseUrl をクライアントに安全に渡せる |
| `google.script.run` は URL に依存しない | 描画フレームが変わっても RPC は正常動作する |

### Dashboard / Run_Log 反映

**不要。** 本フェーズは HTML / GAS コードの変更のみで、シート構造・来院ヘッダ・集計への影響なし。

### 次フェーズ候補

| 候補 | 概要 | 優先度 |
|---|---|---|
| **来院入力フォーム Web 化設計** | 会計区分・部位・ケース入力の Web UI 化可否評価。`readRowNewUI_` 依存コスト調査。設計のみ | 中 |
| **自費 UI 軽微改善** | B7（会計区分）の読み取り方改善 / 保存後に「患者検索に戻る」リンク追加 / エラー表示強化 | 低（実運用で問題が出てから） |
| **受付運用テスト** | 実際の受付業務で 1〜2 週間試用し、使い勝手・速度・エラー頻度を記録する | 高（コード開発より先に実運用へ） |
...
## 🗓 2026-04-22 Web UI 段階移行設計着手

**ドキュメント:** `docs/JREC-01_WebUI_段階移行設計_2026-04-22.md`  
**COMMIT:** `3d28d38`
...
| 項目 | 内容 |
|---|---|
| 方針 | 全面移行ではなく段階移行。制度ロジックは GAS 上で維持 |
| 第1候補 | **患者検索・患者選択 Web UI** |
| 選定理由 | 制度ロジック影響ゼロ / 毎日使う / スマホ対応価値大 / ロールバック容易 |
| Phase 1 | `searchPatients_V3` 新設 + 検索 HTML（シート書き込みなし） |
| Phase 2 | `setPatientAndDate_V3` 追加 + 選択確定でシート B2/B4 書き込み |
| Phase 3 | 来院入力フォーム Web 化可否判断（`readRowNewUI_` 依存解体コスト評価） |
...
## 🗓 2026-04-22 copyInsurerToMaster_V3 ラベル名ベース転記に移行

**COMMIT:** `84aba3b`  
**変更ファイル:** `Ver3_core.js` / `docs/JREC-01_copyInsurerToMaster_label_based_2026-04-22.md`

### 変更サマリ

| 項目 | 変更内容 |
|---|---|
| 患者ID取得 | `getRange(1,2)` B1固定 → `insurerRecord["照会番号"]` ラベル検索 |
| 転記スコープ | `FIELD_MAP` 13項目固定 → 保険者情報ラベルと患者マスタ列名の完全一致で全自動 |
| 表記ゆれ対応 | `normalizeInsurerLabel_` 追加。末尾「 :」「：」を除去して正規化 |
| 新規転記項目 | 記号・枝番・本人家族の別・被保険者氏名（患者マスタに列があれば自動転記） |
| Logger 出力 | 転記済み / 列なし / 値空 / 必須フィールド欠落の4種を出力 |
| セル番地固定参照 | **すべて排除。項目名依存に移行完了** |

### 実機確認ステータス

✅ **照会番号 = 患者ID の運用一致を確認（2026-04-22）** — マイナ確認アプリの照会番号欄にユーザーが院内患者IDを手入力する運用が確定。ID対応リスク解消。

⏳ **残確認2点（シート上）:**
1. 患者マスタに新規列（記号・枝番・本人家族の別・被保険者氏名）が存在するか
2. 実行時 Logger が想定通り出るか（転記済み / 列なし / 値空 / 必須欠落）

詳細: `docs/JREC-01_copyInsurerToMaster_label_based_2026-04-22.md` の「残リスク整理」「次にシートで行う確認手順」セクション

### Dashboard / Run_Log

不要。コード移植のみ。運用フロー・帳票出力・保険計算ロジックへの影響なし。

---

## 🗓 2026-04-22 患者マスタ ヘッダ設計 v1 棚卸し

**ドキュメント:** `docs/JREC-01_患者マスタ_ヘッダ設計_v1_2026-04-22.md`
...
|---|---|---|
| A. 基本属性 | 5 | 現状維持 |
| B. 保険情報（転記対象） | 12 | 現状維持（枝番・有効開始日等が転記成功で確認済み） |
| B'. 保険情報（算出値） | 2 | 現状維持 |
| C. 連絡先 | 2 | 現状維持（電話番号は将来候補） |
| D. 運用メモ | 1 | 現状維持 |
| 保留 | 2 | `氏名カナ` / `資格情報(該当保険)` — 目視確認後に判断 |

### 未確定論点の判定結果（2026-04-22 全件確定）

| 論点 | 結論 |
|---|---|
| P1: `氏名カナ` | **フリガナ統一・列追加不要** — 患者本人カナ = `フリガナ` と同一概念。マイナ確認公式ラベルとの名称違いに過ぎない |
| P2: `資格情報(該当保険)` | **独立列追加不要** — `保険種別`（"協会けんぽ"等）+ `区分` で完全代替可能 |

→ **患者マスタのヘッダ変更・コード変更は一切不要。現行構成を正式採用。**

### Dashboard / Run_Log

不要。設計文書の意味判定のみ。シート変更・コード変更なし。

---

## 🗓 2026-04-21 T-SR-11/12/13/17/18 院長実機確認

### 確認内容

| チェック | 対応TC | 内容 | 結果 |
|---|---|---|---|
| A 再検料 | **T-SR-12** | 4/1初検(1650) / 4/19再検(410+後療505) → 再検料が初検料等欄に正しく表示 | ✅ PASS |
| B 施術終了年月日 | **T-SR-17** | 頸部捻挫・腰部捻挫ともに「年　月　日」空白 → 継続中患者として正常 | ✅ PASS |
| C 負傷名2 | **T-SR-13** | 負傷名②に「腰部 捻挫」表示 → case2の負傷名が正しく出力 | ✅ PASS |
| D 表面情報 | **T-SR-18** | 表面の負傷日時・場所・状況・所見 = case1（頸部捻挫 4/1）の情報 → case2が主表示になっていない | ✅ PASS |
| E 初検抑制 | **T-SR-11** | 4/19はcase2初検日だが初検料=0（再検料410のみ）→ 同月2件目初検抑制が正常動作 | ✅ PASS |

### 確認患者・環境

- 患者: 4月再検あり・case2あり患者 1名
- 1件の実機出力で5項目を同時確認
...
| TC | 内容 | 結果 |
|---|---|---|
| **T-SR-14** | case1のみ2部位 → 負傷名② = 腰部 捻挫 / 日数② = 1 | ✅ PASS |

### 未確認（保留）

| TC | 内容 | 状態 |
|---|---|---|
| T-SR-15 | case1継続中にcase2発生 → 3者整合 | ⏳ 複合ケース発生まで保留 |
| T-SR-16 | case1治癒後case2 → 3者整合 | ⏳ 複合ケース発生まで保留 |

> **施術録 v2 の実機確認は T-SR-15/16 のみ残。** いずれも特殊ケースのため日常診療で該当患者が出た時点で確認する。

### 次にやること

- T-SR-15/16 は複合ケース（case1継続中にcase2発生 / case1治癒後case2）が実際に発生した月の施術録で確認

---

## 🗓 2026-04-20 TC-B04 負傷の原因欄 出力条件バグ修正

### 根本原因

- 旧条件: `row2 is not None and row2["部位1_計"] > 0`
- case1(1部位) + case2(1部位) = 2部位のとき、row2 が存在するだけでトリガー → **2部位でも負傷原因が出力**されていた

### 修正内容

| ファイル | 変更 |
|---|---|
| `write_application.py` | `_slot_filled(r, n)` ヘルパー + 4スロット合計 ≥ 3 に変更 |
| `Ver3_transferData.js` | `V3TR_writeToApplication_()` の `part3HasData` を同条件に変更 |

### 確認済みケース（ローカルテスト PASS）

| ケース | total_parts | 期待 | 結果 |
|---|---|---|---|
| 3部位(2+1) | 3 | TRUE | ✅ |
| 2部位(1+1) | 2 | FALSE | ✅ （旧実装はTRUE→バグ）|
| 2部位(2+0) | 2 | FALSE | ✅ |
| 1部位(1+0) | 1 | FALSE | ✅ |
| 4部位(2+2) | 4 | TRUE | ✅ |
...
- 当月末日自動入力の公式根拠が未確認のため **未実装のまま保留**
- 手書き運用を継続し、根拠確認後に実装する
- SPEC.md §D5 に保留論点として記録済み

### デプロイ

- clasp push: ✅ 完了
- Cloud Run: ✅ revision `00024-5gp`
...
## 🗓 2026-04-20 TC-B06 施術証明欄 日付自動入力 — 不具合修正・正サービスへ再デプロイ

### 根本原因

**D6 コードを誤ったサービス（`write-application`）へデプロイしていた。**

- GAS の `APPGEN_ENDPOINT` Script Property が指すサービス: **`jrec-appgen-server`**（revision 00024-5gp が稼働中）
- 前セッションでのデプロイ先: `write-application`（別サービス、GAS からは一切呼ばれない）
- Cloud Run ログ調査で判明: `write-application` への `/generate` リクエストはゼロ。`jrec-appgen-server` が 2026-04-20 06:12 に本番トラフィックを受けていた。
- D6 コード自体（実装・セル・ロジック）に問題はなく、デプロイ先の誤りが唯一の原因。
...
| `write_application.py` | `clinic_info` dict に `"month"` 追加（batch_write_from_string） |
| `write_application.py` | D6 ブロック: `ws["E56"] = date_str` 直接書込 |
| `write_application.py` | `[D6-DATE] DONE: E56=...` / `[D6-DATE] SKIP: ...` ログ追加 |

### デプロイ

- **正サービス: `jrec-appgen-server`**
- 修正 revision: `00025-4lc`（2026-04-20 デプロイ完了）
- 旧誤デプロイ先 `write-application` は GAS から参照されないため影響なし

### 院長実機確認

- [ ] B案実行 → xlsx E56（ユーザー表示54行目）に `令和X年Y月Z日` が入ることを確認
- [ ] 日付が入らない場合: Cloud Run ログ（`jrec-appgen-server`）で `[D6-DATE]` 行を確認

---

## 🗓 2026-04-20 TC-B05 施術証明欄 自動入力 — 実装完了
...
| `Ver3_transferData.js` | NDJSON `_meta` に3項目を追加（GAS → Cloud Run 伝送） |
...
| 設定シートキー | NdjsonフィールD | xlsx セル | 欄名 |
|---|---|---|---|
| 施術所名 | clinicName | L59 | 施術所名称 |
| 住所 | clinicAddr | L58 | 施術所所在地 |
| 施術者氏名 | clinicPractitioner | L62 | 施術者氏名 |

### デプロイ

- clasp push: ✅ 完了（Ver3_transferData.js）
- Cloud Run: ✅ revision `00021-svr` デプロイ済み（2026-04-20）
  - 本デプロイで 2026-04-06 分の負傷名・負傷の原因 修正（commit 9af6fc4）も同梱

### 院長実施事項（実機確認前に必要）

- **設定シートに「施術者氏名」行を追加**（キー: `施術者氏名`、値: 実際の氏名を記入）
  - 「施術所名」「住所」は既存行を流用（T-SUM-06 で確認済み）
...
| **TC-B04/B05/B06** | ✅ **院長実機確認完了・正式クローズ（2026-04-20）** |
...
| TC | タイトル | 状態 | 優先 | 備考 |
|---|---|---|---|---|
| TC-B01 | 施術終了年月日の転記確認 | ✅ **実質完了・クローズ（2026-03-19）** | — | SPEC.md D1で実xlsx2件確認済み。施術録側も2026-04-19対応済み。「施術証明欄の施術期間」は別欄・別課題 |
| TC-B02 | 継続月数・頻回欄 | ✅ **設計確定・クローズ（2026-03-20）** | — | M31出力停止（意図的）。正本=摘要欄+長期欄（手動）。計算コード `V3TR_calcD2Keizoku_()` 実装済み |
| TC-B03 | 負傷名の左右表記 | ✅ **運用ルール明文化・クローズ（2026-04-20）** | — | コード変更なし。入力側ルール: 体幹=左右不要 / 四肢=右/左を部位名に含める / 両側同時=必記 |
| **TC-B04** | **負傷の原因欄の出力条件修正** | ✅ **実機確認完了・クローズ（2026-04-20）** | — | — |
| **TC-B05** | **施術証明欄の自動入力** | ✅ **実機確認完了・クローズ（2026-04-20）** | — | — |
| **TC-B06** | **施術証明欄 日付（暫定）** | ✅ **実機確認完了・クローズ（2026-04-20）** | — | ⚠️ 公式根拠未確認の暫定運用 |

#### B. 施術録 v2（T-SR-11 以降）

> T-SR-10v2（2026-04-18）: 単一患者・単一ケースで NG-01〜NG-06 全確認済み。
> T-SR-11〜T-SR-18: 実装済み・期待値定義済み。**院長による実機確認未実施。**

| # | 内容 | 状態 | 必要な患者データ |
|---|---|---|---|
| T-SR-11 | case2 同月・初検料抑制 → 裏面列 | ✅ 実機確認済み（2026-04-21） | — |
| T-SR-12 | 再検料の裏面表示 | ✅ 実機確認済み（2026-04-21） | — |
| T-SR-13 | case2 別エピソード → 負傷名2 | ✅ 実機確認済み（2026-04-21） | — |
| T-SR-14 | case1 2部位 → 負傷名2・日数 | ✅ 実機確認済み（2026-04-21） | — |
| T-SR-15 | case1継続中にcase2発生 → 3者整合 | ⏳ 複合ケース発生まで保留 | 複合ケース |
| T-SR-16 | case1治癒後case2 → 3者整合 | ⏳ 複合ケース発生まで保留 | 治癒後別エピソードケース |
| T-SR-17 | 施術終了年月日プレースホルダー | ✅ 実機確認済み（2026-04-21） | — |
| T-SR-18 | 表面 earliest + 裏面 2件目情報 | ✅ 実機確認済み（2026-04-21） | — |
