...
| 7. 完了後 | 全テスト PASS → Phase 2 完了宣言 |

**今日（2026-03-23）にやったこと:**
...
| 対象シート | `メニューマスタ（価格設定）` |
| 識別キー | O列 `menu_id`（英大文字 + アンダースコア） |
| 取得条件 | M列 `確定状況 = "確定"` のみ |
| 失敗時 | fallback（コード内ハードコード4件）で業務継続 |

### JBIZ 側の手動作業（clasp push 後に実施）

| # | 作業 | 方法 |
|---|---|---|
| 1 | `メニューマスタ（価格設定）` の O1 に `menu_id` ヘッダを追加 | GASメニュー「【初回1回】JBIZ menu_id 列追加」を実行 |
| 2 | 未設定行の O列に手動で menu_id を追加 | マッピングに該当しないメニュー名は手動で `SELF_XXX` 形式で追加 |
| 3 | 各行の M列 `確定状況` を確認（ダイアログに表示したい行は "確定" に設定） | 手動 |
| 4 | 下段ルールメモを移行（任意） | GASメニュー「【整理用】JBIZ 会員優待ルール移行」を実行 |

### JBIZ menu_id 命名規則

| menu_id | メニュー名 | 単価 |
|---|---|---|
| `SELF_CHRONIC50` | 慢性ケア手技50分 | 5,500円 |
| `TRAINING_PERSONAL60` | パーソナルトレーニング60分 | 8,800円 |
| `TRAINING_4PASS` | 4回集中コース | 35,200円 |
| `GYM_MONTHLY` | ジム月会費 | 要確認 |
| `SELF_INITIAL_EVAL` | 症状別初回評価 | 3,300円 |
| `INS_BASE` | 保険基本施術 | 保険算定 |
| `INS_OPTION_EXTEND10` | 延長施術10分 | 保険算定 |
| `INS_OPTION_FASCIA_GUN` | 筋膜ガン | 保険算定 |
| `INS_OPTION_HEAT5` | 温熱5分 | 保険算定 |

> 新メニュー追加時は `大区分_詳細` 形式で追加する（例: `SELF_MYOFASCIA30`）

### JBIZ メニューマスタ 列定義（O列追加後）

| 列 | ヘッダ | JREC参照 | 変更ルール |
|---|---|---|---|
| A | 表示順 | ❌ 未使用 | 自由 |
| B | 大区分 | ❌ 未使用 | 意味変更時は本ファイルに記録 |
| C | 小区分 | ❌ 未使用 | 意味変更時は本ファイルに記録 |
| **D** | **メニュー名** | **✅ 使用（表示名）** | **O列 menu_id 設定後に変更する。変更後は過去の自費明細データと名称ズレが生じる（集計影響なし）** |
| E | 内容 | ❌ 未使用 | JBIZ運用上の意味が変わる変更は本ファイルに記録 |
| F | 時間 | ❌ 未使用 | JBIZ運用上の意味が変わる変更は本ファイルに記録 |
| **G** | **一般料金（円）** | **✅ 使用（unitPrice）** | 変更すると次回ダイアログから新単価が反映される |
| H | ジム会員料金（円） | ❌ 未使用（将来対応予定）| JBIZ運用上の意味が変わる変更は本ファイルに記録 |
...
| N | 備考 | ❌ 未使用 | 自由 |
| **O** | **menu_id** | **✅ 使用（識別キー）** | **変更するとJREC保存データとの整合が崩れる。変更不可（追加のみ可）** |

#### 列変更の記録ルール（2026-03-23 決定）

| ルール | 対象 |
|---|---|
| **記録必須（本ファイルへ）** | B・C・E・F・H の「意味・用途が変わる変更」（ヘッダ名変更・区分体系の変更等）|
| **変更前にO列設定を確認** | D列（メニュー名）の変更は O列 menu_id が設定済みであることを先に確認する |
| **変更不可** | O列（menu_id）の既存値変更。JREC自費明細シートの過去データとキーが不一致になる |
...
| 項目 | 影響 | 対応 |
|---|---|---|
| T2-1 / T2-2 / T2-9 | ✅ OK（保存ロジック変更なし） | 再確認不要 |
| T2-3a〜T2-3d | ⚠️ **再確認必要** — ダイアログのメニュー一覧が JBIZ 参照に変わる | JBIZ menu_id 設定後に実施 |
| T2-4〜T2-8 | ⚠️ 再確認推奨 — 保存時の menuId が新形式になる | T2-3 後に続けて実施 |
| T2-10〜T2-12 | ✅ 影響なし（保険算定・申請書ロジック変更なし） | 順序通り実施 |

> JBIZ menu_id が設定されていない状態で T2-3 を実施すると fallback（4件）が表示される。
> 正式には JBIZ 側の手動作業完了後に T2-3 を実施すること。
