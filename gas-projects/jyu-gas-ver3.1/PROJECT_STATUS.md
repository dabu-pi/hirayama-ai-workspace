...
最終更新: 2026-04-22（**患者マスタ ヘッダ設計 v1 棚卸し完了 — 氏名カナ・資格情報(該当保険)の目視確認待ち**）
...
### 実機確認待ち（シート目視のみ・コード変更なし）

1. 保険者情報の `氏名カナ` 行の実値 → 患者本人カナか被保険者カナか確認
2. 保険者情報の `資格情報(該当保険)` 行の実値 → `保険種別` との整合確認

### Dashboard / Run_Log

不要。設計文書作成のみ。シート変更・コード変更なし。
