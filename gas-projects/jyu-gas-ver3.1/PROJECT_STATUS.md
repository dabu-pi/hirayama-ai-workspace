...
最終更新: 2026-04-03（**WS-SR 表示追加修正 v11: 今回セッション分 Markdown整理・一区切り完成化**）
...
**次の作業:** 今回は **Markdown記録のみで一区切り**。必要時のみ、WS-SR の **請求年月日 / 領収年月日 表示実装** から再開する。運用方針は v11 に記録済み。改善候補 I-1 は引き続き保留。
...
| 3. 現状 | **WS-SR 表示追加修正 v11 Markdown整理完了。施療料/後療料分離、①集計欄整理、請求期間2行表示、年付き略記、円付き表示、転帰既定表示まで反映済み。請求年月日・領収年月日は運用方針のみ確定し、実装は保留。** |
| 4. 次の作業候補 | 必要時のみ **請求年月日 / 領収年月日 表示実装** から再開。請求期間セルは同一セル2行構造、金額は円付き、転帰は未入力時 `治癒･中止･転医` 既定表示を前提にする。 |
