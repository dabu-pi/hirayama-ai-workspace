...
最終更新: 2026-03-31（**Phase A/B ジム会員フラグ実機確認完了 + 保存後会計サマリー案A 実装完了**）

---

## ✅ 現在のステータス（2026-03-31 更新）

**Phase 0〜3: 実装完了・実機確認完了 / Phase A/B: 実機確認完了 / 保存後サマリー案A: 実装完了・実機テスト待ち**
...
| **サマリー案A** | **保存後会計サマリー（J55:Q62）+ 領収証レイアウト** | **🔧 実装完了・実機テスト待ち（2026-03-31）**|

**次の作業:** T-SUM-01〜06 を実機確認する。設定シートに施術所名／住所／電話を記入する。
...
### 保存後会計サマリー兼領収証導線 — 案A 実装完了（2026-03-31）
...
| 変更箇所 | 内容 |
|---|---|
| `UI.summary_area = "J55:Q62"` | サマリーエリアのレンジ定数を UI const に追加 |
| `writeSavedSummaryUI_V3_(uiSh, obj)` | 新設。8行×8列レイアウトでサマリー＋領収証を書く |
| `saveVisit_V3` | ⑥ の前に patientDisplayName / selfPayTotal を取得。clearAfterSaveUI 後に writeSavedSummary 呼び出し |
| `clearEntryUI_V3` | `UI.summary_area` を `breakApart().clearContent().clearFormat()` で消す |
| `setupSelfPayValidation_V3_` | `UI.summary_area` を初期化（「来院保存後に表示されます」プレースホルダ）|
| `clearAfterSaveUI_V3_` | **変更なし**（サマリーは次回保存時に上書き）|
...
Row 57: 保険分合計(参考) | ¥xx | ①一部負担金 | ¥xx | ②保険外 | ¥xx | 合計金額(①+②) | ¥xx [薄オレンジ]
...
設定シートの A列に「施術所名」「住所」「電話」、B列に実際の値を記入する。
未設定の場合は「（設定シートに…記入してください）」プレースホルダが表示される。
