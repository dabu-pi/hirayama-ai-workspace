...
### 保存後会計サマリー兼領収証導線 — 案A v2 実装完了（2026-03-31）

設計書: `docs/JREC-01_RECEIPT_SUMMARY_DESIGN_2026-03-31.md`
ステータス: **案A v2 実装済み・clasp push 済み（2026-03-31）— v2 実機テスト待ち**

v1 テスト結果: T-SUM-01〜04 OK / T-SUM-05 全消去だったため値クリアのみに変更 / T-SUM-06 設定不反映だったため全行検索に修正

#### v2 実装内容

| 変更 | v1→v2 |
|---|---|
| 配置 | J55:Q62 → **J2:N22（縦並び21行）** |
| 時刻 | 保存時刻あり → **日付のみ** |
| clearEntryUI_V3 | 全消去 → **`clearSummaryValuesUI_V3_`（値のみ / ラベル・枠は残る）** |
| 設定シート読取 | 最大20行 → **全行検索** |
| 追加関数 | `getClinicInfoFromSettings_V3_` / `clearSummaryValuesUI_V3_` |
| `UI.summary_area` | `"J55:Q62"` → `"J2:N22"` |
...
| No | 操作 | 期待結果 |
|---|---|---|
| T-SUM-01 | 保険のみ患者を保存 | J2:N22 にサマリー表示。①一部負担金=計算値、②保険外=¥0 |
| T-SUM-02 | 保険+自費患者を保存 | 保険外=自費明細合計、合計=①+② |
| T-SUM-03 | 自費のみ患者を保存 | 保険分合計=¥0、一部負担金=¥0、保険外=自費明細合計 |
| T-SUM-04 | 次の患者を保存 | 前回サマリーが新サマリーで上書きされる |
| **T-SUM-05** | 患者画面クリアボタン | **値セル（患者名・日付・金額・visitKey）のみ空欄。ラベル・枠・領収証定型テキストは残る** |
| **T-SUM-06** | 設定シート記入 → 保存 | **Row 19〜21 に施術所名・住所・電話が正しく表示される** |
