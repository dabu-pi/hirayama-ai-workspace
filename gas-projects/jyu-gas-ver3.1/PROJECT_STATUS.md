...
| No | 欄名 | セル | GAS側 | Python側 | 正本候補 | 優先度 |
|---|---|---|---|---|---|---|
| U1 | 都道府県番号 | 未確認（上段）| ❌ 未実装 | ❌ 未実装 | 設定シート固定値（施術機関の都道府県コード2桁） | **高** |
| U2 | 施術機関コード | 未確認（上段）| ❌ 未実装 | ❌ 未実装 | 設定シート固定値（国保連付番のコード） | **高** |
| U3 | 保険種別 | CB8〜CJ11（○付） | ✅ `master.insuranceType` | ✅ `INSURANCE_TYPE_CELLS` で○付 | 保険者番号から自動判定 or 患者マスタ | **DONE** |
| U4 | 単併区分 | 未確認 | ❌ 未実装 | ❌ 未実装 | 固定「単独」（公費併用患者なし前提）。将来は患者マスタに公費区分フィールド追加 | **中** |
| U5 | 本家区分 | 未確認 | ⚠️ `続柄` はtransferColsにあるが申請書セルへの書込なし | ❌ 未実装 | 「続柄+保険者番号+年齢」からコード判定（2本人/6家族/8高一/0高七/6後期） | **中** |
| U6 | 給付割合 | 未確認 | ✅ `V3TR_pickBurdenDigit_` で一部負担金割合(1/2/3)を算出済み | ❌ 給付割合セルへの書込なし | `一部負担金割合` から逆算（10−割合=給付割合: 3割→7, 2割→8, 1割→9） | **中** |
| U7 | 請求区分（新規・継続） | DH31（確認済） | ❌ `row["請求区分"]` 未生成 | ❌ `CELL_MAP["請求区分"]` 定義済みだが `put()` なし | 初検年月日の月=対象月→新規 / 前月以前→継続 / 同月内治癒再発→両方○ | **高** |
...
- 神奈川県=14、など全国共通コード
- 正本: 設定シートで固定値管理。GAS・py 両側でセル番地確認後に追加実装

**U2: 施術機関コード** — 制度定義整理済み ★★☆
- 国保連が受領委任届出後に付番する施術所固有コード
- 桁数は都道府県によって異なる可能性あり（公式統一桁数は未確認）
- 正本: 設定シートで固定値管理

**U4: 単併区分** — 制度定義整理済み ★★☆（2026-03-20）
- 「単独」= 健保/国保単独 / 「2件」= 健保＋地方単独公費1件 / 「3件」= 健保＋地方単独公費2件
- 地方単独事業例: 重度障害者医療費助成・ひとり親家庭等医療費助成・小児医療費助成
- **当面は固定「単独」で実装**。公費併用患者対応時に患者マスタへ「公費区分」フィールドを追加する設計とする

**U5: 本家区分** — 制度定義整理済み ★★☆（2026-03-20）
- 単純な「本人/家族」ではなく、年齢・所得区分を含む**複合コード体系**
- 主要コード: 2本人（70歳未満本人）/ 6家族（70歳未満家族）/ 8高一（前期高齢一般）/ 0高七（前期高齢現役並）/ 6後期（後期高齢: 給付9/8/7割）
- 判定には 続柄・保険者番号（法別番号）・年齢 の3要素が必要。単純な続柄→本人/家族の変換では不十分
- セル番地未確認（テンプレートスキャン要）

**U6: 給付割合** — 制度定義整理済み ★★☆（2026-03-20）
- 申請書には一部負担金割合ではなく給付割合（7/8/9）を記載する
- 3割負担=7割給付、2割負担=8割給付、1割負担=9割給付
- GAS: `V3TR_pickBurdenDigit_` が一部負担金割合(1/2/3)を返すため、逆算で 10-割合=給付割合
- セル番地未確認。py側の書込処理なし

**U7: 請求区分（新規・継続）** — 制度定義整理済み ★★★（2026-03-20）
- DH31（確認済）への書込が必要
- 複数の国保連記載要領で判定基準が一致
- 同月内治癒再発ケース（case2が同月内新規）では両方○が必要 → case2 の初検月判定が必要
