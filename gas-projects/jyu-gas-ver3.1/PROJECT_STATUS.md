...
最終更新: 2026-04-06（**過去日再読み込み・転帰後付け更新 実装完了**）

---

## 🆕 2026-04-06 過去日再読み込み・転帰後付け更新

**追加関数:**

| 関数 | メニュー | 用途 |
|---|---|---|
| `reloadVisitToUI_V3` | 柔整管理 > 当日内容を再読み込み | 患者ID+来院日で来院ケースを検索し UI に復元 |
| `updateOutcomeFromUI_V3` | 柔整管理 > 転帰を更新 | UI の転帰・終了日のみを来院ケースに後付け書き込み |
| `writeExactCaseSrcToUI_` | —（内部） | ケースデータをそのまま UI 2行にセット（autofill とは別ロジック） |
| `readCaseKubun_` | —（内部） | visitKey+caseNo で区分を補完取得 |

**設計方針:** 金額・初検/再検ロジックは一切再計算しない。転帰・終了日の2項目だけを安全に書き込む。

**詳細:** `docs/JREC-01_reload_and_outcome_2026-04-06.md`

**次アクション:** clasp push → 柔整管理メニューから実機確認。振り返り→転帰更新の導線を確認。
