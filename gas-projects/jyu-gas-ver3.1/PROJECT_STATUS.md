...
テストデータは `needCheck=TRUE` かつ施術日 2999 年のため安全。  
月次申請には影響しないが、スプレッドシートで手動削除を推奨。

**削除手順:**
- 来院ケースシート: 施術日=2999-12-31 の行を削除
- 来院ヘッダシート: visitKey=（検証用実在ID）_2999-12-31 の行を削除
- 削除後: `npm run test:jyu:web25 -- --project=chromium` で W2.5-4 が「新規保存 PASS」に変わることを確認

**W2.5-4 の状態:** テストデータ未削除時も DUPLICATE_VISIT を検出して PASS（テスト自体は壊れない）。
