...
| U2 | 施術機関コード | ✅ **CZ2**（CZ2:DV3）| ✅ 登録記号番号から数字部分導出→CZ2（暫定）| ✅ derive_clinic_code()→CZ2（暫定）| 設定「登録記号番号」の数字部分（暫定: 先頭協/契+ハイフン除去）| ~~中~~ **DONE** |
...
**U2: 施術機関コード** — ✅ 実装済み（2026-03-20）★★☆（★暫定運用★）
- 国保連が受領委任届出後に付番する施術所固有コード
- **暫定ルール**: 設定シート「登録記号番号」から先頭「協/契」除去 + ハイフン除去 → 数字部分を CZ2 に書込
...
- セル番地確定済み（DP8/DP11）。py側の書込処理は未実装（次タスク候補）
