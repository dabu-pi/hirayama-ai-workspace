...
最終更新: 2026-04-26（**Phase A-1 SC1 暫定対応完了 — 前回自費明細フォールバック追加・clasp push 済み**）
...
| SC1 | 前回自費明細がある患者で反映される | 🔶 設計課題により保留（§2026-04-26 セクション参照） |
| SC2 | 前回自費明細がない患者で通常ダイアログが開く | ⬜ 実受付テスト待ち（記録テンプレート準備済み） |

### SC2 実受付テスト記録欄（結果出たら記入）
...
1. **次回受付日に SC2 の実受付テスト**を実施 → 上記テンプレートに記入 → §8-D に転記
2. 2026-04-30 頃に GAS 実行ログ（エラー率）確認
3. SC2 結果を踏まえ 2026-05-07 目安で Phase A 完了判定 → Phase B 移行可否を判断
