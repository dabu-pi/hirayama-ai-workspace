...
最終更新: 2026-03-22（会計区分ゲート実装 — 自費のみ時の保険保存防止・レセプト事故防止）
...
**🔲 Phase 2 + 会計区分ゲート 実機確認待ち（院長 clasp push 後に実施）:**

| # | テスト内容 | 確認ポイント |
|---|---|---|
| T2-1 | 自費1件（M001/5500円）保存 | 自費明細 1行 / D7=メニュー名 / F7=5500 / H8="1件保存済" / 来院ヘッダ更新 |
| T2-2 | 自費2件（M001/M010）保存 | 自費明細 2行 / D7="〇〇ほか1件" / F7=14300 / H8="2件保存済" |
| T2-3 | 保存後 再保存（金額変更）| 旧行全削除 → 新行追記 / D7/F7/H8 再計算 / 来院ヘッダ上書き |
| T2-4 | 価格マスタ変更後の過去データ | 自費明細の unitPrice は保存時点のまま（snapshot 維持）|
| T2-5 | 会計区分=保険のみ・自費なし | H8="未入力"（ダイアログ未起動）/ D7/F7 空欄 / 警告なしで saveVisit 続行 |
...
| **T2-9** | **自費のみ + 保険継続 case あり** | **confirm 警告が出ること / OK 後: 来院ケースシートに追加なし / 保険算定なし / ヘッダ保険列=0** |
| **T2-10** | **自費のみ + 保険データ入力なし** | **警告なし / 自費明細のみ保存 / 来院ヘッダ保険列=0 / 自費明細列は正常記録** |
| **T2-11** | **保険のみ + H8="1件保存済"** | **confirm 警告が出ること / OK 後: 通常通り保険保存 / ヘッダ自費金額=0** |
| **T2-12** | **保険+自費（通常ケース）** | **両方保存 / ヘッダ保険列+自費列ともに正常値 / 来院ケースあり** |

- 優先度: 最優先
- ブランチ: `feature/auto-dev-phase3-loop`
- 最新コミット: `5cd9a44`（docs更新後に新コミット予定）
