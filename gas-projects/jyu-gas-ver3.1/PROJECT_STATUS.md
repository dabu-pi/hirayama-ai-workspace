...
最終更新: 2026-04-01（**WS-SR 施術録導線ワークストリーム開始 — 構造・方針確定**）
...
**次の作業:** WS-SR 施術録導線の未確定論点①②③を確認・決定 → 実装フェーズ設計。 / 改善候補 I-1（来院ヘッダ列順整理）は保留中。設定シートへの施術所名／住所／電話記入は運用開始済み（T-SUM-06 PASS 確認）。
...
|---|---|
| 管理単位 | 1患者 × 1負傷系列 |
| 転記サイクル | 月次で追記・転記・確定保存 |
| 出力形式 | テンプレート形式 → 最終 PDF 保存 |
| 転記後元データ | 削除しない。Archive化 / 退避前提で再検討 |
| 優先範囲 | 保険施術録を先行。自費混在は初版後回し |

### 未確定論点（実装前に詰める）

| # | 論点 |
|---|---|
| ① | 施術経過所見を月次施術録へどう出すか（日別 vs 月別まとめ） |
| ② | 裏面の日別欄の元データ（来院ヘッダ / 施術明細 / 中間データ層） |
| ③ | 負傷系列の切り方（再負傷・別部位追加・継続中の扱い） |
| ④ | 自費混在ケースの施術録上の扱い |
| ⑤ | 出力テンプレート形式の最終選択 |
| ⑥ | Archive 化のタイミングと保管場所 |

### 次のアクション（WS-SR）

1. 未確定論点①②③を人間と確認・決定する
2. 出力テンプレート形式を決定する（⑤）
3. 実装フェーズ設計（JREC-01 の Phase C or 新 Phase として設計）
4. 実装着手

### 参照先

- 詳細設計方針: `docs/施術録導線/JREC-01_施術録導線_設計方針.md`
- 関連既存ドキュメント: `docs/JREC-01_初期評価テンプレ.md` / `docs/JREC-01_再診経過記録テンプレ.md`
