...
最終更新: 2026-03-20（中断前記録整理・申請書欄修正完了）
...
| 優先 | タスク | 理由 |
|---|---|---|
| 1 | D5 施術証明欄・委任欄 | 毎回手書き運用のため優先度は低いが未実装 |
| 2 | U5 後期高齢者の制度確認 | 保険種別=6 の場合の本家区分記載方式が未確認。空欄のままか記載必要か |
| 3 | D4 3部位ケースの追加確認 | row2["部位1_計"] > 0 の判定が実案件で正しく動くか確認 |
| 4 | 高7 の "⓪" 表示確認 | Unicode U+24EA が帳票上で正しく表示されるか未確認 |
| 5 | D2 継続月数・頻回 | 制度定義（連続月数・月10回以上の連続月数）の再確認後に実装 |
| 6 | U2 施術機関コードの公式確認 | 暫定ルール（先頭協/契除去・ハイフン保持）の公式根拠が未確認 |
...
| GitHub（コード） | ✅ 反映済み | commit b6a7c79 / push 済み（feature/auto-dev-phase3-loop）|
