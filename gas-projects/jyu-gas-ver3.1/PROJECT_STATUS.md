...
最終更新: 2026-04-02（**WS-SR テンプレート確認・sandbox コード作成完了 — 人間が sandbox 実行・PDF レイアウト確認後に実装フェーズへ**）

---

## ✅ 現在のステータス（2026-04-01 更新）
...
**次の作業:** WS-SR — **sandbox GAS（`claude-sandbox/WS_SR_sandbox.js`）を GAS エディタで実行 → ログ確認 → PDF レイアウト確認** → 確認結果を `JREC-01_施術録GDocs技術検証.md` に追記 → 実装フェーズ設計。 / 改善候補 I-1（来院ヘッダ列順整理）は保留中。
...
**ステータス: テンプレート確認・sandbox コード作成完了（2026-04-02更新） / sandbox 実行は人間が実施 / 実装未着手**
...
### テンプレート技術検証（2026-04-02 確認済み）

| 項目 | 内容 |
|---|---|
| テンプレートファイル | 施術録様式参考例（Google Docs） |
| テンプレート ID | `1Tcq8kcwFfIzFixGF54xFoWyZcNz7IsgjYsT8NqV0mnY` |
| 出力フォルダ ID | `1XMx2e1ufCRqp7bhpfRRjvPDyXCESL83V` |
| sandbox コード | `claude-sandbox/WS_SR_sandbox.js` |
| 差し込みアプローチ | 表面固定欄: replaceText / 裏面日別行: テーブルセル直接書き込み |
| 技術検証 MD | `docs/施術録導線/JREC-01_施術録GDocs技術検証.md` |

### 残未確定論点

| # | 論点 | 状況 |
|---|---|---|
| ① | GAS → Google Docs API の技術可否 | **sandbox 実行で確認予定**（人間が実施）|
| ② | テーブル構造（Index・行・セル位置）の確定 | sandbox ログから特定 |
| ③ | 整復料/後療料の列振り分けロジック（byomei + kubun で振り分け） | 設計要 |
| ④ | `shoken` / `keikaNow` どちらを主転記列にするか | 設計要 |
| ⑤ | Archive化のタイミングと Drive フォルダ設計 | 未確定 |
| ⑥ | 自費混在ケースの扱い | 初版スコープ外 |

### 次のアクション（WS-SR）

1. **sandbox 実行（人間）:** `WS_SR_sandbox.js` を GAS エディタで実行 → ログ確認 → PDF レイアウト確認
2. **確認結果記録:** チェックリスト C-01〜C-08 + 判断 D-01〜D-05 を `JREC-01_施術録GDocs技術検証.md` に追記
3. 整復料/後療料の列振り分けロジック確定
4. `shoken` / `keikaNow` 主転記列確定
5. 実装フェーズ設計（WS-SR Phase 1）
6. 実装着手
...
- **技術検証メモ: `docs/施術録導線/JREC-01_施術録GDocs技術検証.md`** ← 今回作成
- 既存テンプレ: `docs/JREC-01_初期評価テンプレ.md` / `docs/JREC-01_再診経過記録テンプレ.md`
