...
最終更新: 2026-04-02（**WS-SR T-SR-01〜T-SR-09 完了 — `Ver3_shuRecorder.js` 実装完了・`Ver3_core.js` メニュー追加 / 次: T-SR-10 clasp push + 手動テスト**）
...
**次の作業:** WS-SR — **T-SR-10: `clasp push` → 手動テスト（1患者・2026-04）** / 改善候補 I-1 保留中。
...
**ステータス: T-SR-01〜T-SR-09 完了（2026-04-02更新） / 次: T-SR-10 clasp push + 手動テスト**
...
| **T-SR-10** | **clasp push + 手動テスト（1患者・2026-04）** | **実機確認** | **⏳ 次のアクション** |
| T-SR-11 | テスト結果を TESTCASES.md に記録 | ドキュメント | 未着手 |

### 次のアクション（WS-SR）

**T-SR-10: clasp push + 手動テスト**

1. GAS スクリプトエディタへ反映（clasp push）:
   ```
   cd C:\hirayama-ai-workspace\workspace\gas-projects\jyu-gas-ver3.1
   clasp push
   ```
2. スプレッドシートをリロードして「柔整ツール」メニューに「施術録を出力」が表示されることを確認
3. 「施術録を出力」を選択 → ダイアログで患者IDと年月（2026-04）を入力 → 実行
4. Drive の `施術録/2026-04/` フォルダに Docs + PDF が出力されることを確認
5. PDF を開いてレイアウト・差し込み値が正しいことを目視確認
6. 問題なければ T-SR-11 として TESTCASES.md に記録

**SR_SUM_COL 調整が必要な場合:**
- ①集計行のセル位置（`SR_SUM_COL` 定数）は目視推定値。実行後ログで実際の列数を確認し、`Ver3_shuRecorder.js` 先頭の定数を修正して再 clasp push すること。
