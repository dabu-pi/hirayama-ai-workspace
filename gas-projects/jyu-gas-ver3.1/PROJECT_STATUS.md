...
- 状態: 稼働中（B案確認完了。D2制度定義確定・実装待ち。残課題: D5/U5後期高齢者/暫定運用欄公式確認）
- 優先度: 最優先
...
**B案（申請書生成）は完全に動作確認済み。D2制度定義確定・仕様固定（2026-03-20）。次の作業はD2実装（GAS+Python+Cloud Run再デプロイ）。**

| カテゴリ | 状態 | 詳細 |
|---|---|---|
| B案申請書生成 | ✅ **本番稼働可能** | 3件生成/エラー0件/全患者クリーン（2026-03-20確認済み）|
| 申請書上段欄 U1〜U7 | ✅ 全欄実装済み | U2/U5/U6 は暫定運用（制度公式確認が残課題）|
| D2 継続月数・頻回 | 🔧 **実装待ち** | 制度定義確定・仕様固定済み（2026-03-20）。caseKey×部位単位・月10回以上連続月数・6か月目以降「6」固定・頻回後は0.5継続。GAS+Python+Cloud Run再デプロイが必要 |
...
| 優先 | タスク | 種別 | 理由 |
|---|---|---|---|
| 1 | D2 継続月数・頻回 | **実装（GAS+Python+再デプロイ）** | 仕様確定済み。月10回以上連続月数・rawContMonths/freqStartedの内部分離・M31表示値「6」固定等を実装 |
| 2 | D5 施術証明欄・委任欄 | 未実装（低優先）| 手書き運用で当面は問題なし |
| 3 | U5 後期高齢者の本家区分 | 保留（制度未確認）| 保険種別=6 / 75歳以上の場合の本家区分記載方式が未確認。空欄運用継続 |
...
| D2 | 継続月数・頻回欄が未対応 | 🔧 **制度定義確定・仕様固定済み（2026-03-20）**— caseKey×部位単位で月10回以上の連続月数を計算。rawContMonths/freqStartedを内部保持し表示値は1〜5/「6」固定で分離。頻回開始後は治癒等まで0.5継続。長期理由書（3か月超）は別制度として手動対応 | `Ver3_transferData.js` + `write_application.py` + Cloud Run再デプロイ | **高（実装待ち）** |
...
**残課題なし。D1はクローズ。D2は制度定義・仕様確定済み・実装待ち。**

**D2 — 継続月数・頻回（🔧 仕様固定・実装待ち 2026-03-20）**
...
#### 制度定義・仕様確定結果（2026-03-20）✅ 完了

根拠: 厚生労働省通知・地方厚生局確認

| 論点 | 確定内容 |
|---|---|
| 判定単位 | **caseKey × 部位**（エピソード×部位ごとに独立）|
| 「継続月数」の定義 | **月10回以上の施術を行った連続月数**（単純経過月数ではない）|
| 計算ロジック | `calcMonthsElapsed_V3_` は単純経過月数のため不十分。`buildMonthlyVisitCounts_V3_` ベースの新ロジックが必要 |
| 表示値 | 1〜5か月目: 実際の連続月数 / 6か月目以降: **「6」固定** |
| 頻回の成立 | 月10回以上×5か月連続 → 翌月から0.5逓減 |
| 頻回開始後 | 月10回未満でも治癒・中止・転医まで0.5継続（解除不可）|
| 長期施術継続理由書 | 3か月超の別制度 → 手動対応（システム外）|

#### 実装内容（次タスク）

| ファイル | 変更内容 |
|---|---|
| `Ver3_transferData.js` | `rawContMonths`/`freqStarted`計算ロジック追加 + `row["経過"]` 生成 |
| `write_application.py` | `CELL_MAP["経過"] = "M31"` + 書込処理追加 |
