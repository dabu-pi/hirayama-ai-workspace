...
最終更新: 2026-04-06（**申請書正式経路整理 + Cloud Run 再デプロイ待ち**）

---

## ⚠️ 2026-04-06 Cloud Run 再デプロイ待ち（申請書正式帳票）

### 状況

| 経路 | 修正状態 | 備考 |
|---|---|---|
| GAS テンプレ直反映（`V3TR_writeToApplication_`） | ✅ 反映済み（clasp push 7dadd97） | 確認用 |
| Cloud Run xlsx 生成（`write_application.py`） | ❌ **未反映**（再デプロイ要） | **正式帳票の正本経路** |

`write_application.py` の修正（9af6fc4）は Cloud Run に未反映。
正式帳票出力経路では旧仕様（番号なし・並び順旧・1行連結）のままで動いている。

### 再デプロイ手順

`docs/JREC-01_CloudRun_デプロイ手順.md` の §6〜§7 を実行すること（docker build → push → gcloud run deploy）。

### 申請書正式帳票の位置づけ（確定）

| 区分 | メニュー | 出力先 |
|---|---|---|
| **正式帳票** | 管理者用 > **申請書を生成して Drive に保存** | Cloud Run → xlsx → Drive |
| 確認用 | 柔整管理 > 申請書テンプレに反映（画面確認用） | スプレッドシート直接書き込み |
| 中間データ作成 | 柔整管理 > 申請書転記データを作成 | 申請書_転記データ シートのみ |

正式帳票の正本は `application_template.xlsx`（シート「新　様式第5号」）から生成した xlsx。
スプレッドシート直反映は確認用・デバッグ用の補助機能であり、正式帳票の正本ではない。
