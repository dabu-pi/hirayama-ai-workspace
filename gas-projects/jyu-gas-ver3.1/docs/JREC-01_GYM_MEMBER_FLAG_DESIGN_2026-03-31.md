...
作成日: 2026-03-31
ステータス: **Phase A/B 実機確認OK・運用可能（2026-03-31）— Phase C は未着手**
実施者: Claude Code
...
### テスト観点・実機確認結果（2026-03-31 全件OK）

| # | テスト内容 | 期待結果 | 実機確認 |
|---|---|---|---|
| T-GYM-01 | B5 チェックあり（会員）で保存 → 来院ヘッダの「ジム会員フラグ」列を確認 | TRUE が記録されている | ✅ OK |
| T-GYM-02 | B5 チェックなし（非会員）で保存 → 来院ヘッダの「ジム会員フラグ」列を確認 | FALSE が記録されている | ✅ OK |
| T-GYM-03 | 保存後に B5 が FALSE に戻っているか確認 | clearAfterSaveUI_V3_ で FALSE リセット | ✅ OK |
| T-GYM-04 | 「患者画面クリア（入力のみ）」実行後に B5 が FALSE になっているか確認 | clearEntryUI_V3 で FALSE リセット | ✅ OK |
...
| # | テスト内容 | 期待結果 | 実機確認 |
|---|---|---|---|
| T-GYM-B1 | B5 チェックあり → 自費明細ダイアログを開く | バッジ「🏋 ジム会員料金 適用中」が表示される | ✅ OK |
| T-GYM-B2 | B5 チェックなし → 自費明細ダイアログを開く | バッジが表示されない | ✅ OK |
| T-GYM-B3 | B5 チェックあり → JBIZ H列に会員料金が設定されているメニューを選択 | 会員料金が単価欄に入る | ✅ OK |
| T-GYM-B4 | B5 チェックあり → JBIZ H列が 0（未設定）のメニューを選択 | G列の一般料金が入る（フォールバック）| ✅ OK |
| T-GYM-B5 | B5 チェックなし → 同じメニューを選択 | G列の一般料金が入る | ✅ OK |

**運用注意（仕様）:** JBIZ H列が未入力のメニューは会員でも G列へフォールバック。不具合ではなく意図した設計。会員価格を使いたいメニューから順に H列を整備すること。
...
| フェーズ | 状態 |
|---|---|
| Phase A: UI チェックボックス + 来院ヘッダ保存 | ✅ 完了・実機OK |
| Phase B: 自費ダイアログ 一般/会員料金切替 | ✅ 完了・実機OK |
| Phase C: 患者マスタ連携（自動セット）| 🔲 未着手 |

**現在の運用状態:** 来院時に B5 を手動チェックすれば会員料金が自費ダイアログに反映される。運用可能。

### Phase C 設計メモ（将来実装用）

- 患者マスタに「ジム会員」列を追加し boolean 値を持たせる
- 患者 ID 入力時（`onEdit` or `autofillFromPreviousVisit_V3`）にマスタを参照して B5 を自動セット
- **当日入会・当日変更のため、B5 による当日UI上書きは Phase A/B の実装をそのまま維持**
- 方針: 「患者マスタ既定値 + 当日 UI 上書き」の2段構え
- 着手判断: 運用データが蓄積し、手動チェックの手間が問題になったとき

## 次に院長が判断すべき1点

> **Phase C（患者マスタ連携）をいつ着手するか？**
> Phase A/B は完了・運用可能。Phase C は手動チェックの負担が増えたタイミングで判断する。
...
| 2026-03-31 | T-GYM-B1〜B5 全件 OK。Phase B 実機確認完了。ステータスを「Phase A/B 実機確認OK・運用可能」に更新。テスト結果列追加・Phase C 設計メモ・現在地セクション追加 |
