...
制度の概要:
- 骨折・脱臼の整復および不全骨折の固定に際し金属副子等を使用した場合に加算（1,000円）
- 後療時の交換は**2回まで**加算可（§18.3）
- 使用・交換した日を申請書摘要欄と施術録に記載

⚠️ 解釈確認待ち論点（2026-03-17）:

| 論点 | §18.3 の記述 | 付録A の記述 | 有力解釈 |
|---|---|---|---|
| 合計算定回数 | 「後療時の交換は2回まで」 | 「3回まで」 | 初検日1回 + 後療時交換2回 = 計3回（未確定） |
| 回数カウント単位 | 未定義 | 未定義 | caseKey 全期間 or 月単位（未確定） |

根拠候補: **令和6年5月29日付 留意事項通知・疑義解釈資料**（厚生労働省保険局医療課）
→ 疑義解釈で「初検時1回・後療時交換2回まで = 合計3回」が明示されているか確認が必要

確認が取れるまで:
- 金属副子等加算の実装は保留
- 設定シートへのキー登録も保留
- 対象症例は手動で申請書摘要欄に記載して対応する
