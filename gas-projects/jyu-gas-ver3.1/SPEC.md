...
実装方針（Phase 1 / Phase 2 に分割）:
- **Phase 1（基本加算）**: UIチェック追加 + 設定シートキー `金属副子等加算`=1,000 + `calcOnePartAmount_V3_` 拡張（傷病種別チェックのみ）
- **Phase 2（回数制限）**: caseKey単位の過去算定回数集計（来院ヘッダ参照）→ 後療時交換2回超で base=0 + 要確認
- Phase 1 は単独で価値あり。Phase 2 は `buildMonthlyVisitCounts_V3_` と同類の履歴参照が必要で別タスク扱い
