...
/** =========================
 * Phase 3: Gmail 下書き作成（3-1〜3-3）
 * ========================= */

/**
 * Phase 3 メイン: 見積済み（R列あり）・下書き未作成（U列空）の行に Gmail 下書きを作成する
 * - 3-1: freee IV API から見積 PDF 取得を試みる
 * - 3-2: PDF 添付 + 返信テンプレートで下書き保存（送信しない）
 * - 3-3: PDF 取得失敗時は freee 見積 URL を本文に記載してフォールバック
 */
function freee_phase3_createGmailDrafts() {
  const sheet = getSheet_();
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return;

  const numCols = Math.max(sheet.getLastColumn(), CFG.COL_DRAFT_CREATED_AT);
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const draftCreatedAt = row[CFG.COL_DRAFT_CREATED_AT - 1];
    if (draftCreatedAt) continue;

    const gmailMsgId   = String(row[CFG.COL_MESSAGE_ID - 1]    || '').trim();
    const mailLink     = String(row[CFG.COL_MAIL_LINK - 1]     || '').trim();
    const customerName = String(row[CFG.COL_CUSTOMER_NAME - 1] || '').trim();
    const subject      = String(row[CFG.COL_DESC - 1]          || '').trim();
    const quotedAt     = row[CFG.COL_QUOTED_AT - 1];

    try {
      createQuotationDraft_({ gmailMsgId, mailLink, quotationId, customerName, subject, quotedAt });
      sheet.getRange(rowIndex, CFG.COL_DRAFT_CREATED_AT).setValue(new Date());
      console.log(`✅ Row ${rowIndex} (${customerName}): Gmail 下書き作成完了`);
    } catch (err) {
      console.error(`❌ Row ${rowIndex}: Gmail 下書き作成失敗:`, err && err.stack ? err.stack : err);
    }
  }
}
...
/**
 * 1行分の下書きを作成する
 * @param {string} p.gmailMsgId  - A列 Gmail Message-ID
 * @param {string} p.mailLink    - O列 Gmail スレッドリンク
 * @param {string} p.quotationId - R列 freee quotation_id
 */
function createQuotationDraft_(p) {
  // 3-1: PDF 取得（失敗しても 3-3 URL フォールバックで継続）
  let pdfBlob = null;
  try {
    pdfBlob = fetchQuotationPdf_(p.quotationId);
    console.log(`PDF取得成功: quotation_id=${p.quotationId} (${pdfBlob.getBytes().length} bytes)`);
  } catch (err) {
    console.warn(`PDF取得失敗 → URLフォールバック: ${err && err.message ? err.message : err}`);
  }

  const quotationUrl = buildFreeeQuotationUrl_(p.quotationId);
  const quotedDate   = normalizeDateYmd_(p.quotedAt) || todayYmd_();
  const body         = buildDraftReplyBody_(p.customerName, p.subject, quotedDate, quotationUrl, !!pdfBlob);
  const draftSubject = p.subject ? 'Re: ' + p.subject : '御見積書のご送付（' + p.customerName + '）';

  // 3-2 / 3-3: 元スレッドに返信下書き → 取得失敗時は hawk 宛に新規下書き
  const thread = findGmailThread_(p.gmailMsgId, p.mailLink);
  if (thread) {
    if (pdfBlob) {
      thread.createDraftReply(body, { attachments: [pdfBlob] });
    } else {
      thread.createDraftReply(body);
    }
  } else {
    const to = 'hawk@pop13.odn.ne.jp';
    if (pdfBlob) {
      GmailApp.createDraft(to, draftSubject, body, { attachments: [pdfBlob] });
    } else {
      GmailApp.createDraft(to, draftSubject, body);
    }
  }
}

/**
 * Gmail スレッドを探す（複数フォールバック）
 * 優先順位: O列メールリンク → A列 rfc822msgid 検索 → A列 Gmail 内部 ID
 */
function findGmailThread_(gmailMsgId, mailLink) {
  // 1) O列（メールリンク）からスレッドIDを抽出
  if (mailLink) {
    const m = String(mailLink).match(/[#/]([a-f0-9]{8,})\b/);
    if (m) {
      try {
        const t = GmailApp.getThreadById(m[1]);
        if (t) return t;
      } catch (_) {}
    }
  }
  ...
  const cleanId = String(gmailMsgId).replace(/^<|>$/g, '');

  // 2) rfc822msgid 検索（A列が RFC Message-ID ヘッダ値の場合）
  try {
    const threads = GmailApp.search('rfc822msgid:' + cleanId, 0, 1);
    if (threads && threads.length > 0) return threads[0];
  } catch (_) {}

  // 3) Gmail 内部メッセージ ID として試行（A列が getMessageById 用 ID の場合）
  try {
    const msg = GmailApp.getMessageById(cleanId);
    if (msg) return msg.getThread();
  } catch (_) {}
...
/**
 * freee IV API から見積 PDF を取得して Blob を返す（3-1）
 * エンドポイント: GET /iv/quotations/{id}/pdf
 */
function fetchQuotationPdf_(quotationId) {
  const token = getValidAccessToken_();
  const url   = CFG.FREEE_API_BASE + CFG.IV_BASE_PATH + '/quotations/' + quotationId + '/pdf';
  ...
  if (code !== 200) {
    throw new Error('PDF取得失敗 HTTP ' + code + ': ' + res.getContentText().slice(0, 200));
  }

  return res.getBlob().setName('見積書_' + quotationId + '.pdf');
}

/**
 * freee 見積書の管理画面 URL（ログイン要）
 * PDF 添付失敗時（3-3）の暫定案として、また PDF 添付時の補足情報として常に本文に付与する
 */
function buildFreeeQuotationUrl_(quotationId) {
  return 'https://invoice.freee.co.jp/quotations/' + quotationId;
}

/**
 * Gmail 下書き本文を生成する
 * @param {boolean} hasPdf - true=PDF添付あり(3-2)、false=URLのみ(3-3)
 */
function buildDraftReplyBody_(customerName, subject, quotedDate, quotationUrl, hasPdf) {
  const salutation = customerName ? customerName + '様' : 'ご担当者様';
  const attachmentLine = hasPdf
    ? '添付のとおり、御見積書をお送りいたします。'
    : '御見積書を以下の URL からご確認いただけます。\n' + quotationUrl;

  return [
    salutation,
    '',
    'いつもお世話になっております。',
    'この度はお見積りのご依頼をいただきまして、誠にありがとうございます。',
    '',
    attachmentLine,
    'ご確認のほど、よろしくお願いいたします。',
    '',
...
function freee_runAll() {
  ...
  try {
    freee_phase3_createGmailDrafts();
  } catch (err) {
...
/**
 * デバッグ: Phase 3 の本文プレビュー・PDF取得テスト（下書き保存はしない）
 */
function freee_testPhase3() {
  const sheet = getSheet_();
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) { console.log('データなし'); return; }
  ...
  let target = null;
  for (let i = 0; i < values.length; i++) {
    const quotationId = String(values[i][CFG.COL_FREEE_QUOTATION_ID - 1] || '').trim();
    if (quotationId) { target = { index: i + 2, data: values[i] }; break; }
  }
  if (!target) { console.log('R列（quotation_id）がある行が見つかりません'); return; }
  ...
  console.log('=== Phase3 テスト: 行' + target.index + ' / quotation_id=' + quotationId + ' ===');
  console.log('customer:', customerName, '/ subject:', subject);
  console.log('gmailMsgId:', gmailMsgId, '/ mailLink:', mailLink);

  // PDF 取得テスト
  let pdfBlob = null;
  try {
    pdfBlob = fetchQuotationPdf_(quotationId);
    console.log('✅ PDF取得成功:', pdfBlob.getName(), pdfBlob.getBytes().length + ' bytes');
  } catch (err) {
    console.warn('⚠️ PDF取得失敗 →', err && err.message ? err.message : err);
  }

  // スレッド検索テスト
  const thread = findGmailThread_(gmailMsgId, mailLink);
  console.log('スレッド検索:', thread ? '✅ 見つかりました → 返信下書き' : '⚠️ 見つからず → 新規下書き to hawk@pop13.odn.ne.jp');

  // 本文プレビュー
  const quotationUrl = buildFreeeQuotationUrl_(quotationId);
  const quotedDate   = normalizeDateYmd_(quotedAt) || todayYmd_();
  const body = buildDraftReplyBody_(customerName, subject, quotedDate, quotationUrl, !!pdfBlob);
  console.log('=== 下書き本文プレビュー ===\n' + body + '\n===========================');
  console.log('※ テストなので下書きは保存していません。本番: freee_phase3_createGmailDrafts()');
}
