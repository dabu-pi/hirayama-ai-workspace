...
def create_sheet1(wb):
    ...
    widths = [14, 9, 20, 16, 18, 25, 23, 25, 15, 15]
    style_header(ws, 1, headers, widths)

    # ドロップダウン
    add_dropdown(ws, "D", 2, 201, '"A_内需安定,B_IT・DX,C_製造・精密,D_医療,E_金融,TBD"')
...
def create_sheet2(wb):
    ...
    widths = [14, 9, 18, 16, 13, 13, 15, 11, 12, 11, 10, 14, 11, 25, 25, 18, 14, 25]
    style_header(ws, 1, headers, widths)

    # ドロップダウン
    add_dropdown(ws, "D", 2, 501, '"A_内需安定,B_IT・DX,C_製造・精密,D_医療,E_金融"')
...
def create_sheet4(wb):
    ...
    widths = [14, 9, 18, 16, 26, 25, 15, 14, 13, 26, 9, 13, 25, 28]
    style_header(ws, 1, headers, widths)
