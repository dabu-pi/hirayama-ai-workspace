...
### `ai-invest` → **ARCHIVED** ✅ 実施済み（2026-05-02）

- 退避先: `_archive_workspace_cleanup_20260502/ai-invest/`
- git-tracked 14ファイルをインデックスからも削除し commit 済み
- PROJECTS.md の AINV-07 を `inactive / archived_2026-05-02` に更新済み
- **スクリプト未更新（要注意）:** 以下のスクリプトが `ai-invest/` パスを参照している。退避後に実行すると stale 参照が残るため、次回実行前に更新要。
...
## NO_ACTION_TAKEN（調査セッション時点）

この調査レポートの初版（2026-05-02 調査セッション）では移動・削除・退避は実施しなかった。
その後の実施セッション（2026-05-02 同日）で以下を実行済み:
- `ai-invest/` を `_archive_workspace_cleanup_20260502/ai-invest/` へ退避
- PROJECTS.md 更新（AINV-07 archived, MSRB-01 追加, JASSESS-01 追加）
