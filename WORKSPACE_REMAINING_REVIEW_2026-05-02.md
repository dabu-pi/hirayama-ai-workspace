...
### `projects/machine-sales-rebuild` → **KEEP（現在地維持）** ✅ 実施済み
...
- PROJECTS.md に **MSRB-01** として正式登録済み（2026-05-02 追加）

### `ai-invest` → **KEEP（方針変更 2026-05-02）** ⚠️ フォルダー復元が未完了

**方針:** オーナーが「今後使用予定あり」と判断。AINV-07 として継続。アーカイブしない。

**現在の状態（不整合あり）:**
- 物理フォルダー: `_archive_workspace_cleanup_20260502/ai-invest/` に存在（workspace には **ない**）
- git: `0f5f1ba` で削除済み（14ファイルが git 履歴上は削除された状態）
- PROJECTS.md: `workspace/ai-invest` / `registration_candidate` / `keep` に更新済み（本セッション）

**要対応（フォルダー復元）:**
1. `Move-Item "C:\hirayama-ai-workspace\_archive_workspace_cleanup_20260502\ai-invest" "C:\hirayama-ai-workspace\workspace\ai-invest"`
2. `git checkout 455bcfb -- ai-invest/` で git の削除を取り消す（または `git revert 0f5f1ba` で関連コミットを revert）
3. 復元後に `git status` で確認・`git commit` で復元をコミット

**スクリプト参照は影響なし（復元後は正常動作する）:**
- `scripts/update-extra-projects-metadata.mjs`（AINV-07 specUrl）
- `scripts/update-live-projects-sheet-metadata.mjs`（directory: 'ai-invest'）
- `scripts/sync-workspace-to-drive.ps1`（ai-invest/PROJECT_STATUS.md, README.md）

### `msk-assessment-platform` → **KEEP（退避禁止）** ✅ 変更なし

- 稼働中の接骨院用 Google Sheet + Apps Script と連携済み
- PROJECTS.md に **JASSESS-01** として正式登録済み（2026-05-02 追加）
- scripts/ から複数の参照がある（退避すると破損する）
...
| セッション | 内容 |
|---|---|
| 2026-05-02 調査セッション | 調査・レポート作成のみ。フォルダー操作なし |
| 2026-05-02 実施セッション① | `ai-invest/` を archive へ退避・PROJECTS.md 更新（AINV-07 archived, MSRB-01/JASSESS-01 追加） |
| 2026-05-02 実施セッション② | **方針変更。** ai-invest を KEEP に変更。PROJECTS.md で AINV-07 を `registration_candidate / keep` に戻す。フォルダー復元は別途要対応。 |
