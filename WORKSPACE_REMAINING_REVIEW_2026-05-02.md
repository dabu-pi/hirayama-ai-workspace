...
### `ai-invest` → **KEEP ✅ 復元完了（2026-05-02）**

**方針:** AINV-07。Phase 0 完了・Phase 1（ペーパートレード）未着手。

**最終状態（整合済み）:**
- 物理フォルダー: `workspace/ai-invest/` に存在（復元済み）
- git: commit `a4cf696` で14ファイルを再追加
- PROJECTS.md: `workspace/ai-invest` / `registration_candidate` / `keep`
- archive backup: `_archive_workspace_cleanup_20260502/ai-invest_restore_backup_20260502/`（手動削除可）

**スクリプト参照は正常（復元後は全て動作する）:**
- `scripts/update-extra-projects-metadata.mjs`
...
| 2026-05-02 実施セッション② | 方針変更。ai-invest を KEEP に変更。PROJECTS.md AINV-07 を `registration_candidate/keep` に戻す。 |
| 2026-05-02 実施セッション③ | **ai-invest 復元完了。** archive → workspace へ Move-Item、git add + commit `a4cf696`。workspace 整理完了。 |
