...
最終更新: 2026-03-23（腰痛評価シートシステム JEVAL-01 追加）
...
6. [腰痛評価シートシステム JEVAL-01](#6-腰痛評価シートシステム-jeval-01)
...
## 6. 腰痛評価シートシステム JEVAL-01

**ディレクトリ:** `low-back-assessment/`
**ステータス:** 設計完了・実装待ち（Phase 1 着手前）
**最終確認日:** 2026-03-23

### 目的

接骨院での腰痛評価を標準化し、評価→施術方針→コメント→患者説明まで一貫して出力できるスプレッドシートシステム。
将来的に施術者判定 + ルールベース + Claude AI の3層判定に拡張できる基盤を設計段階から確保する。
...
├── SPEC.md                 # 仕様書・評価ツール根拠
├── SHEET_DESIGN.md         # シート構成・列設計案
├── LOGIC.md                # ルールベース判定ロジック案
├── COMMENT_DESIGN.md       # コメント生成設計案
├── CLINICAL_OPERATION.md   # 臨床運用設計案
└── gas/
    └── setup_sheets.js     # スプレッドシート自動生成GAS
```
...
| ツール | 目的 | エビデンス |
|---|---|---|
| NRS | 痛みの数値評価 | 感度・再現性ともに高い（VASと同等） |
| RMDQ-10（短縮版） | 機能障害評価 | 国際標準・臨床変化の検出に優れる |
| STarT簡易版（9項目） | 慢性化リスクスクリーニング | RCTで有効性確認（Hill 2011, Lancet） |
| PSFS | 患者個人の機能目標評価 | 治療目標共有に有用 |
| 赤旗スクリーニング | 重篤疾患除外 | 診療ガイドライン共通必須項目 |

### 設計方針

- **評価者差の低減:** プルダウン・チェックボックス中心。テキスト自由入力は所見欄のみ
- **将来AI連携の基盤:** AI判定用列を初版から確保。Claude APIへのJSON出力シートを準備
- **安全ネット:** 馬尾症候群・赤旗を入力時に自動アラート（背景色変化）
...
|---|---|
| 言語 | Google Apps Script (JavaScript) |
| プラットフォーム | Google スプレッドシート（8シート構成） |
| 将来連携 | Claude API（claude-sonnet-4-6）、clasp管理 |
...
| 2 | 腰痛評価シート JEVAL-01 | 臨床現場での即時活用・治療家育成の基盤 |
