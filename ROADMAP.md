...
| # | タスク | 内容 | ステータス |
|---|---|---|---|
| 2-1 | OAuth再構築 | `reset()` でトークン全削除 → `redirect_uri` をGAS WebアプリURL `/exec` と完全一致させて再認証 | ⏸ 待機 |
| 2-2 | 取引先検索・自動作成 | B列（顧客名）→ freee `partner_id` を自動解決。未登録なら自動作成してP列に保存 | ⏸ 待機 |
| 2-3 | 見積書POSTの完成 | Q列（lines_json）をもとに freee `/iv/api/1/quotations` へPOST | ⏸ 待機 |
| 2-4 | 成功時の自動記録 | G列（見積日）・R列（quotation_id）をシートに保存 | ⏸ 待機 |
| 2-5 | 未決事項の確認 | hawk全メール処理か件名フィルタか / 発生日ルール / 明細不足時の例外設計 | ⏸ 待機 |

#### フェーズ 3 — Gmail下書き作成

| # | タスク | 内容 | ステータス |
|---|---|---|---|
| 3-1 | 見積PDF取得 | freee API から見積PDF を取得 | ⏸ 待機 |
| 3-2 | 返信メール下書き生成 | PDF添付 + 返信文テンプレートで下書き保存（送信しない） | ⏸ 待機 |
| 3-3 | 暫定案（PDF困難な場合） | freee 見積URL を本文に貼る形式 | ⏸ 待機 |
...
| **3月下旬** | freee自動化: OAuth再構築完了・フェーズ2（見積書作成）動作確認 | ⏸ 待機 |
| **4月上旬** | freee自動化: フェーズ3（Gmail下書き）完成 | ⏸ 待機 |
