...
> 作成: 2026-05-02 / 更新: 2026-05-02（Phase S-5 方針変更）  
> ステータス: **Phase S-5 方針変更済み**  
> 前提: `docs/ACCOUNT_SETTINGS_ROADMAP.md` と合わせて読むこと

## ⚠️ 方針変更ノート（Phase S-5 追記）

**最終方式は「申請フロー（S-4）」ではなく「自己責任即時削除（S-7）」に変更された。**

| フェーズ | 内容 | 状態 |
|---------|------|------|
| S-4 | 申請フロー UI 実装（ユーザーが申請 → 管理者審査） | 実装済み・本番デプロイ保留 |
| S-5 | 即時削除方式の調査・設計 | 完了 |
| S-6 | 即時削除に向けた DB / RLS / admin 整備 | 未着手 |
| S-7 | 自己責任削除 UI 実装 | 未着手 |

**S-4 の UI は S-7 実装時に置き換える。管理者向け `/admin/account-deletion-requests` 画面は維持する。**

詳細設計は `docs/SELF_SERVICE_ACCOUNT_DELETE_DESIGN.md` を参照。
