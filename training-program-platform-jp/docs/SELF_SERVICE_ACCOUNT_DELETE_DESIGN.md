...
| membership_status の扱い | ジム会員管理用のカラム。アプリ削除では**絶対に変更しない** |
| cancelled_at の扱い | ジム退会日。アプリ削除では**絶対に変更しない** |
| auth.users 物理削除の影響 | auth.users を削除しても、ジム会員契約・会費・受付業務には影響しない |
| S-7 soft delete の課題 | auth.users にメールが残るため、再登録・ログイン挙動が分かりにくい |
| S-8 方針 | auth.users の物理削除を正式採用。同メールでの再登録が自然なフローになる |
| 削除ログの保持 | `account_deletion_logs` に snapshot（email / display_name / membership_status）を**auth.users 削除前**に必ず残す |
