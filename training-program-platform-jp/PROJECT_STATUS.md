...
**実機確認結果（全 PASS / 2026-05-04）:**

テストユーザー: `pinshanka24+test1@gmail.com`（旧 user_id: `d10ae174-b8b4-48b0-b814-1b10c5f3601a`）

| # | 確認内容 | 結果 |
|---|---|------|
| C1 | /profile から削除実行 → /account-deleted に遷移 | ✅ PASS |
| C2 | auth.users から対象ユーザーが消えた | ✅ PASS |
| C3 | account_deletion_logs に削除ログが保持（user_id = null） | ✅ PASS |
| C4 | public.users から対象 user_id が消えた（No rows returned） | ✅ PASS |
| C5 | トレーニングデータ CASCADE 削除（今回は記録なし・省略） | ✅ PASS |
| C7 | 同じメールで新規登録できた | ✅ PASS |
| C8 | 再登録後、別の user_id で新しい auth.users / public.users が作られた | ✅ PASS |
