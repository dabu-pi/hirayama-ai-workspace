...
- [ ] /gym でメッセージブロックが表示される
- [ ] 経過日数に応じてボーダー色が変わる
- [ ] 記録なしユーザーで「まだトレーニング記録がありません」と表示される
- [ ] 既存の最終トレーニング日・今月の回数表示が壊れていない

### REMAINING_RISKS

- スマホ実機でレイアウト確認が必要（左ボーダーのサイズ感）
- タイムゾーン計算（JST変換）は `toLocaleString` 依存 — 環境によって差異がないか確認
