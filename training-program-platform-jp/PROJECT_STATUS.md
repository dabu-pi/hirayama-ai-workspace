...
以下はブラウザの admin ログインが必要なた���確認待ち。確認できたら各行を更新すること。

| 確認項目 | 結果 |
|---|---|
| `/admin/members` が正常表示される | 未確認 |
| サマリーカードが表示・数値が整合する | 未確認 |
| 最終ログイン / 開始回数 / 完了回数 / 最終T が表示される | 未確認 |
| スマホ幅（390px前後）で横スクロールできる | 未確認 |
| member_name 編集・保存が動作する | 未確認 |
| status 変更・保存が動作する | 未確認 |
| 検索・フィルターが動作する | 未確認 |
...
live 確認で見るべき点:
1. Vercel ログに `buildPreviousDisplayMap: embedded query failed` が**出ていない**こと
2. resume flow で WorkoutScreen が表示されること（StartSessionScreen に落ちていないこと）
3. セット行に前回値（例: `80kg x 5`）が出ていること（`-` だけではないこと）
4. ブラウザコンソールに supabase query error がないこと
