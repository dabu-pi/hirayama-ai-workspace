...
| 項目 | 確定方針 |
|------|---------|
| アカウント管理の分離 | トレーニングアプリのアカウント管理とジム会員管理は**完全に別管理** |
| アプリ利用の任意性 | ジム会員全員がアプリを使うわけではない。アプリ利用は任意 |
| アプリ削除の独立性 | アプリ削除は、ジム退会・休会・会費停止とは**完全に無関係** |
| membership_status | ジム会員管理用。アプリ削除では**絶対に変更しない** |
| cancelled_at | ジム退会日。アプリ削除では**絶対に変更しない** |
| auth.users 物理削除の影響 | ジム会員契約・会費・受付業務には**影響しない** |
| S-7 soft delete の課題 | auth.users にメールが残るため再登録・ログイン挙動が分かりにくい。S-8 で解消 |
| 削除ログ | auth.users 削除**前**に `account_deletion_logs` に snapshot を必ず残す |

**背景:**
S-7 の soft delete（app_deleted_at）方式では、auth.users が残るため同メールでの重複登録リスクや /login ループが発生した。
アプリ削除はジム運営と完全に独立しているため、auth.users の物理削除に移行しても問題ない。
...
**本番 DB constraint 確認結果（2026-05-03 確認）:**

| constraint_name | table_name | column_name | on_delete |
|----------------|------------|-------------|-----------|
| account_deletion_requests_reviewed_by_fkey | account_deletion_requests | reviewed_by | NO ACTION |
| account_deletion_requests_user_id_fkey | account_deletion_requests | user_id | NO ACTION |
| billing_cutoff_records_confirmed_by_fkey | billing_cutoff_records | confirmed_by | NO ACTION |
| membership_pause_requests_reviewed_by_fkey | membership_pause_requests | reviewed_by | NO ACTION |
| membership_pause_requests_user_id_fkey | membership_pause_requests | user_id | NO ACTION |
| workout_session_exercises_user_exercise_id_fkey | workout_session_exercises | user_exercise_id | RESTRICT |

**作成した migration ファイル:**
`supabase/migrations/20260503_000037_s8_fix_fk_for_auth_delete.sql`

**migration の内容:**
1. account_deletion_requests.user_id → DROP NOT NULL → FK: ON DELETE SET NULL
2. account_deletion_requests.reviewed_by → FK: ON DELETE SET NULL
3. membership_pause_requests.user_id → DROP NOT NULL → FK: ON DELETE SET NULL
4. membership_pause_requests.reviewed_by → FK: ON DELETE SET NULL
5. billing_cutoff_records.confirmed_by → FK: ON DELETE SET NULL
6. workout_session_exercises.user_exercise_id → FK: ON DELETE CASCADE

**本番 DB 適用結果（2026-05-03 確認済み）:**

| constraint_name | on_delete 変更後 |
|----------------|----------------|
| account_deletion_requests_user_id_fkey | ✅ SET NULL |
| account_deletion_requests_reviewed_by_fkey | ✅ SET NULL |
| membership_pause_requests_user_id_fkey | ✅ SET NULL |
| membership_pause_requests_reviewed_by_fkey | ✅ SET NULL |
| billing_cutoff_records_confirmed_by_fkey | ✅ SET NULL |
| workout_session_exercises_user_exercise_id_fkey | ✅ CASCADE |

nullable 確認:
- account_deletion_requests.user_id → ✅ YES
- membership_pause_requests.user_id → ✅ YES

**selfDeleteAccount() S-8 実装内容（2026-05-03）:**

| ステップ | 内容 |
|---------|------|
| 1 | display_name / membership_status をスナップショット取得 |
| 2 | account_deletion_logs に INSERT（削除前に必ず保存） |
| 3 | admin.auth.admin.deleteUser(user.id) で物理削除 |

削除されるもの: auth.users / public.users / 全セッション・セット・enrollment / user_exercises  
SET NULL になるもの: account_deletion_logs.user_id / 申請記録 / gym記録の user_id  
変更しないもの: membership_status / cancelled_at（ジム会員管理は別）

**typecheck / build:** エラーなし ✅ / Compiled successfully ✅

**再登録・メール未達の追加調査メモ（2026-05-04）:**

| 確認内容 | 結果 |
|---------|------|
| pinshanka24+test1@gmail.com を Supabase Authentication から手動削除後、同じメールで新規登録 | ✅ 登録できた・確認メールも届いた |
| S-7 方式（auth.users を残す）での同メール再登録 | ❌ メール未達（既存ユーザー扱いになるためと推定） |
| Supabase Auth のメール送信機能自体 | ✅ 正常動作（gmail 宛は届く） |

**判断:**
- S-7 で auth.users が残っていた場合、同メールで再登録すると Supabase が「既存ユーザーへの確認メール再送」扱いにする。この場合、確認メールが届かないケースがある（設定依存）
- S-8 で auth.users を物理削除すれば、同メールアドレスを完全に「新規」として扱えることが確認された
- S-8 方向性は妥当と確認
- `poohsan.milkyway.solar@docomo.ne.jp` への未達は、キャリアメール側の受信制限・迷惑メール設定・URL 付きメール拒否の可能性が高い（Supabase 側の問題ではない）

**実機確認手順（テストユーザーのみ）:**

| # | 確認内容 | 期待結果 |
|---|---|---------|
| C1 | /profile でアカウント削除を実行 | /account-deleted に遷移する |
| C2 | auth.users が削除されている | Supabase Dashboard → Authentication → Users で確認 |
| C3 | account_deletion_logs に履歴が残っている | user_id が null、email_snapshot が入っている |
| C4 | public.users 行が存在しない | Table Editor で確認 |
| C5 | workout_sessions 等のトレーニングデータが存在しない | CASCADE 削除の確認 |
| C6 | membership_status / cancelled_at が変わっていない | ジム会員データは変更なし（対象ユーザーの行自体が消えるため確認不要） |
| C7 | 削除後、同じメールアドレスで新規登録できる | /login → 新規登録フローで確認 |
| C8 | 新規登録後、別の user_id で新しい public.users 行が作られる | Table Editor で確認 |
| C9 | 既存の別ユーザーの動作に影響なし | 通常ユーザーで /gym /train 等が正常動作 |
| C10 | admin 画面に影響なし | /admin/members が正常表示 |
