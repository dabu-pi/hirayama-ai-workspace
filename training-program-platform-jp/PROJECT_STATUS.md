...
**まだ実施していないこと:**
- Supabase 本番 DB への migration 適用
- `selfDeleteAccount()` Server Action の変更
- typecheck / build

**次のステップ:**
1. SQL 内容を確認後、Supabase Dashboard で migration を適用
2. `selfDeleteAccount()` を更新（app_deleted_at 削除 → deleteUser() 追加）
3. typecheck / build
4. テストユーザーで実機確認
