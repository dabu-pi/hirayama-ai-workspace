...
### STATUS: ✅ 本番反映済み — Vercel production build success / 実機確認待ち (2026-05-02)

**Vercel:**
- commit `4b51bbd` → Vercel state: `success`（06:20:34Z）
- production URL に反映済み

**実機確認チェックリスト:**

| # | 確認内容 | 結果 |
|---|---|---|
| T1 | トレーニング画面を開くと topBar 下に設定バーが表示される | ⬜ |
| T2 | 初期値が 1:30 になっている | ⬜ |
| T3 | 2:00 プリセットをタップ → 休憩ボタン表示が 2:00 に変わる | ⬜ |
| T4 | セット完了後、休憩タイマーが設定値（2:00）からカウントを開始する | ⬜ |
| T5 | タイマー動作中は設定バーが非表示になる | ⬜ |
| T6 | +15秒 / −15秒 ボタンが動作する | ⬜ |
| T7 | ページリロード後も設定値が残る（localStorage 永続確認） | ⬜ |
| T8 | トレーニング完了処理・次のワークアウト導線が壊れていない | ⬜ |
| T9 | スマホ幅で表示が崩れていない | ⬜ |
