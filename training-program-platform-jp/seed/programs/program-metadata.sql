...
-- Run after the target programs already exist.
--
-- Targets:
--   - gzclp-base
...
-- public program library. It may be re-run safely.
...
begin
  ...
  -- 3. current assignments (authoritative rewrite for target programs)
  delete from public.program_tag_assignments
  where program_id in (prog_gzclp, prog_starting_strength);
  ...
  raise notice 'Seed complete: program metadata assigned for gzclp-base and starting-strength-base.';
end;
$$;
...
-- where p.slug in ('gzclp-base', 'starting-strength-base')
