 ...
 *   active  → paused  (prevents status='active' + archived_at IS NOT NULL, which
 *                       blocks the unique active enrollment index and hides the
 *                       enrollment from /train without releasing the slot)
...
export async function POST(_request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    // When archiving an active enrollment, also set status='paused'.
    // Leaving status='active' with archived_at IS NOT NULL creates an orphaned row
    // that (a) is invisible to /train (which filters archived_at IS NULL) and
    // (b) occupies the unique active-enrollment slot, blocking new enrollments.
    const statusUpdate = enrollment.status === "active" ? "paused" : enrollment.status;

    const { error: updateError } = await dbClient
      .from("program_enrollments")
      .update({ archived_at: now, status: statusUpdate })
      .eq("id", params.enrollmentId)
      .eq("user_id", userId);
