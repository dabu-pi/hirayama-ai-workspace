...
| **休憩タイマー任意変更** | **✅ LIVE_CHECK PASS / CLOSED (2026-05-02)** | **プリセット4種(1:00/1:30/2:00/3:00) + ±15秒調整。localStorage保存。DB変更なし** |
...
| **S-8** | **auth.users 物理削除対応** | アプリ削除はジム退会と無関係。membership_status/cancelled_at は変更しない。FK migration（BLOCK 6箇所）+ `deleteUser()` + S-7 soft delete からの置き換え | **✅ 調査完了・実装承認待ち (2026-05-03)** |
...
- **Phase M-B**: `membership_pause_requests` migration + `/admin/pause-requests` ✅ MAIN FLOW PASS — 休会申請アプリ受付 ⛔ 停止 (2026-04-29) — 受付対応に変更
- **Phase M-C**: 退会申請アプリ受付 ⛔ 停止 ✅ LIVE_CHECK PASS (2026-04-29) — 窓口受付に運用変更。migration 000031 は未適用保留
- **Phase M-D**: `/profile` ステータスバッジ・受付案内統合 + `/gym` 休会申請中通知除去 ✅ LIVE_CHECK PASS (2026-04-29)
- **Phase M 全体: ✅ CLOSED (2026-04-29)** — 休会・退会はアプリ申請なし、受付対応に統一

### UX 修正

- **セット完了チェック高速化** ✅ LIVE_CHECK PASS (2026-04-29) — 重量入力即チェック/連続チェック/リロード保存/Finish 全項目PASS
...
- **アーカイブ連鎖修正 + getTrainFallbackView Strategy 2 削除** ✅ CLOSED (2026-04-29) — LIVE_CHECK PASS / program-stats と /train の整合確認済み
