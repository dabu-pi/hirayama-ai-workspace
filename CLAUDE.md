...
スプレッドシート（Google Sheets）の列構成・シート追加・数式設計を変更したときは、必ず以下を行う。

| 手順 | 内容 |
|---|---|
| 1. 仕様をローカルに記録 | `慢性疼痛_管理表_STATUS.md`（または該当プロジェクトの仕様ファイル）の列定義・数式を更新する |
| 2. commit & push | 仕様ファイルの更新を必ずコミットしてGitHubに反映する |
| 3. 実装はSheets側で | 実際のセル変更・ARRAYFORMULA設置はSheets上で手動またはGASで行う |
