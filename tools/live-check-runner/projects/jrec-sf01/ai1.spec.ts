 ...
 *     → frameLocator ではなく page.locator() を使用する
 *     → smoke.spec.ts の page.title() が GAS タイトルを返すことで確認済み
 ...
 *   - AI1-7 は GAS /dev への clasp push が Phase AI-1 以降であることが前提
 */
...
test.describe(`JREC-SF01 AI1-1: 新規患者登録 — 職業・既往歴欄 [auth: ${HAS_AUTH ? "あり" : "なし"}]`, () => {
  ...
  test("AI1-1a: newPatient — #occupation 入力欄が存在する", async ({ page }) => {
    ...
    // GAS /dev はトップレベルページとして描画される → page.locator() を使用
    await expect(page.locator('#occupation')).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
  ...
  test("AI1-1b: newPatient — #medicalHistory 入力欄が存在する", async ({ page }) => {
    ...
    await expect(page.locator('#medicalHistory')).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
  ...
  test("AI1-1c: newPatient — 「AI補助判定用情報」セクションタイトルが存在する", async ({ page }) => {
    ...
    // patient-form.html のセクションタイトル
    await expect(page.getByText("AI補助判定用情報", { exact: false })).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
});
...
test.describe(`JREC-SF01 AI1-3: カルテ入力 — 患者情報参照欄 [auth: ${HAS_AUTH ? "あり" : "なし"}]`, () => {
  test("AI1-3: visitForm — 患者情報参照欄の存在確認 [patientIdForVisitForm 要設定]", async ({ page }) => {
    ...
    // visit-form.html の patient info ブロック: 性別・年齢・職業・既往歴
    // セレクタが合わない場合は GAS HTML を確認して更新する
    await expect(page.locator('[id*="patientInfo"], .patient-info-block').first()).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
});
...
test.describe(`JREC-SF01 AI1-4: カルテ入力 — 受傷起点・今回追記既往歴欄 [auth: ${HAS_AUTH ? "あり" : "なし"}]`, () => {
  ...
  test("AI1-4a: visitForm — #injuryTrigger 入力欄が存在する [patientIdForVisitForm 要設定]", async ({ page }) => {
    ...
    await expect(page.locator('#injuryTrigger')).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
  ...
  test("AI1-4b: visitForm — #relatedHistoryNote 入力欄が存在する [patientIdForVisitForm 要設定]", async ({ page }) => {
    ...
    await expect(page.locator('#relatedHistoryNote')).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
});
...
test.describe(`JREC-SF01 AI1-7: 会計導線 — dailyCheckout 到達確認 [auth: ${HAS_AUTH ? "あり" : "なし"}]`, () => {
  ...
  test("AI1-7: dailyCheckout — 日付フォーム(#dateForm)が描画される", async ({ page }) => {
    ...
    await expect(page.locator('#dateForm')).toBeVisible({ timeout: LOAD_TIMEOUT });
  });
