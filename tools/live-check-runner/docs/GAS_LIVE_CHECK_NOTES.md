...
## GAS /dev のフレーム構造（2026-05-02 LC-3 調査で判明）

### 結論: GAS /dev はトップレベルページとして描画される

認証済み（auth.json 有効）状態で GAS `/dev` にアクセスした場合:
- **コンテンツはトップレベルページとして描画される**（googleusercontent.com iframe の中ではない）
- `page.title()` が GAS アプリのタイトル（"JREC-SF01 自費カルテ・会計"）を返すことで確認済み
- GAS タイトルが `document.title` に直接反映されている
...
// 誤った書き方（frameLocator は GAS /dev では不要）
// const frame = page.frameLocator('iframe[src*="googleusercontent"]').first();
// await expect(frame.locator('#occupation')).toBeVisible();
```

### なぜ iframe アプローチが誤りだったか

LC-3 初回実装で `frameLocator('iframe[src*="googleusercontent"]')` を使用したところ:
- AI1-7 `frame.locator('body')` は PASS（別の偶発的な iframe の body を掴んでいた）
- AI1-1 `frame.locator('#occupation')` は FAIL（GAS コンテンツはその iframe にない）

smoke.spec.ts の `page.title()` が正しく GAS タイトルを返していた事実から、
GAS コンテンツがトップレベルにあることを逆算して判明。

### /exec URL との違いに注意

| URL | フレーム構造 | DOM アクセス |
|---|---|---|
| `/dev` (認証済み) | トップレベル直接描画 | `page.locator()` |
| `/exec` (公開) | googleusercontent.com iframe 内 | `frameLocator()` が必要な可能性あり |

`/exec` URL を LiveCheck する場合は改めてフレーム構造を確認すること。
