...
| 🟡 中 | ドロップダウン検証未設定 | Lists の語彙はあるがデータ検証が各シートに未設定（疑い） | 人間 |
| 🟡 中 | Run_Log 記録の習慣化 | 手動記録フロー開始済み（2026-03-08〜）。2週間継続が目標 | 人間 |
| 🟢 低 | Metrics 業績KPI未入力 | 患者数・保険請求額等は構造のみで数値が未入力 | 人間 |
...
- [ ] Task_Queue・Projects の各セルにドロップダウン検証設定済み
- [ ] Dashboard シートの数式参照（QUERY/FILTER）が正しく動作している（詳細確認中）
- [ ] Run_Log に毎セッション終了後 1行追記する習慣が確立している（2026-03-08 開始・継続中）
- [ ] 手動更新フローを 1〜2 週間継続できている
