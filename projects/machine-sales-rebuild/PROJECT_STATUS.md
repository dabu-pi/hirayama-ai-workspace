...
- `frontend/public-preview/` にローカル確認用フロントを追加し、通常商品 63件 / placeholder 3件の分岐表示を実装した
- `baseImageUrl` 本番第一候補を「同一サイト配下の静的配信」とし、差し替え箇所を `frontend/public-preview/config.js` に集約した
...
1. 同一サイト配下の静的ディレクトリをどこに置くか確定する
2. placeholder 3件の実画像回収へ進むか判断する
3. 非公開商品や売却済み商品の画像を公開側だけで拾えるか確認する
4. `strongdepot-product-manager` の PHP / WordPress 側資産を回収する
5. ローカルフロントを本番寄り配置へ移すか判断する
...
uv run python -m scripts.generate_frontend_integration_check
...
- `data/output/frontend_integration_check.csv`
