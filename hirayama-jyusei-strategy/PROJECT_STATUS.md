...
- メニューマスター再編の方針文書化まで完了（`menu/menu-master-redesign.md` 新規作成）
  ...
  - 新設候補（症状別 運動療法 初回評価 / ジム月会員コース分岐）は料金・内容が未確定のため文書のみ
  - シートへの反映は未確定事項の確定後に実施
...
4. メニューマスター再編: `症状別 運動療法 初回評価`（腰痛・首肩こり・膝）の内容・時間・料金を設計する
5. メニューマスター再編: ジム月会員コース（ライト/スタンダード/プレミアム）の月額・利用条件を決める
6. 上記 4〜5 が確定したら `価格設定` シートのブロック整理を反映する
